...
Public Class CommodityType
    ...
    Protected Overrides Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        '    LoadTreeByName()
        'Else
        '    lblTitle.Text = "Commodity Type Selection"
        '    LoadTree()
    ...
    Private Sub PopulateRootLevel()
        PopulateNodes(objComm.getCommodityTypeList, TreeView1.Nodes)
    End Sub

    Private Sub PopulateNodes(ByVal dt As DataTable, ByVal nodes As TreeNodeCollection)
        For Each dr As DataRow In dt.Rows
            Dim tn As New TreeNode()
            tn.Text = dr("CT_NAME").ToString()
            tn.Value = dr("CT_ID").ToString()
            'tn.Value = dr("CT_LAST_LVL").ToString()
            If dr("CT_LAST_LVL").ToString() Then
                'tn.ImageUrl = "font-size: 8pt;font-family: Verdana;background:white;color:Magenta"
                tn.Text = "<b> » " & tn.Text & "</b>"
                tn.NavigateUrl = "javascript:pass_target('" & tn.Value & "', '" & tn.Text & "');"
            End If
    ...
    Private Sub PopulateSubLevel(ByVal parentid As Integer, ByVal parentNode As TreeNode)
        PopulateNodes(objComm.getCommodityTypeListChild(parentid), parentNode.ChildNodes)
    End Sub
    ...
    '                            If tnC.ID = viewstate("selectedId") Or _
    '                                (tnC.ID.Substring(0, 6) = CStr(viewstate("selectedId")).Substring(0, 6) And tnC.ID.Substring(6, 2) = "00") Or _
    '                                (tnC.ID.Substring(0, 4) = CStr(viewstate("selectedId")).Substring(0, 4) And tnC.ID.Substring(4, 4) = "0000") Or _
