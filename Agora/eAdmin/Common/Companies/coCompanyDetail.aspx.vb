...
Public Class coCompanyDetail
    ...
    Protected WithEvents ddlResident As System.Web.UI.WebControls.DropDownList
...
#End Region
    ...
    Private Sub Populate()
        ...
        If Not objCompDetails Is Nothing Then
            ...
            Dim strResidentType = ""
            If ViewState("Mode") = "regappr" Then
                strResidentType = objDb.GetVal("select ifnull(cm_resident,'') from company_mstr where cm_coy_id ='" & ViewState("ComID") & "'")
            Else
                strResidentType = objDb.GetVal("select ifnull(cm_resident,'') from company_mstr where cm_coy_id ='" & Session("CompanyIdToken") & "'")
            End If
            If strResidentType = "N" Then
                ddlResident.SelectedValue = "N"
            Else
                ddlResident.SelectedValue = "Y"
            End If
            'End
            ...
            If dsAppPackage.Tables(0).Rows.Count > 0 Then
                ...
                For i = 0 To dsAppPackage.Tables(0).Rows.Count - 1
                    If dsAppPackage.Tables(0).Rows(i).Item("chk") = "Y" Then
                        ...
                        ''Copy File
                        Dim clsFileMgmt As New FileManagement

                        Dim strfilename As String = objDb.GetVal("select cm_coy_logo from company_mstr where cm_coy_id = '" & Common.Parse(txtCoyID.Text) & "' and cm_coy_name = '" & Common.Parse(txtCoyName.Text) & "'")
                        Dim strSourcePath = clsFileMgmt.getBasePath(EnumUploadFrom.FrontOff, ConfigurationSettings.AppSettings(dsAppPackage.Tables(0).Rows(i).Item("ap_app_id") & "Path")) & "\CoyLogo\" & strfilename
                        Dim strDestinationPath = System.AppDomain.CurrentDomain.BaseDirectory.Replace("\eAdmin\", "") _
                                                & "/FileMgnt" _
                                                & "\CoyLogo\" _
                                                & strfilename
                        If System.IO.File.Exists(strSourcePath) AndAlso Not System.IO.File.Exists(strDestinationPath) Then
            ...
            If pAppPkg.Length > 0 Then
                ...
                For i = 0 To pAppPkg.Length - 1
                    If pAppPkg(i) <> "" Then
                        ...
                        Dim objDB As New  EAD.DBCom(ConfigurationSettings.AppSettings(pAppPkg(i) & "Path"))
    ...
    End Sub
    'Zulham 19072018 - PAMB
    Private Sub updateResidentType()
        Dim strSQL, strCompId, Query(0) As String
        strSQL = "UPDATE COMPANY_MSTR SET CM_RESIDENT = '" & ddlResident.SelectedValue & "' WHERE CM_COY_ID = '" & txtCoyID.Text & "'"
