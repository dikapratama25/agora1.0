        ...
        If Not Page.IsPostBack Then
            ...
            If viewstate("Modes") = "F" Then
                ...
                ElseIf viewstate("Status") = "E" Then
                    lblMsg.Text = ": Please change password due to password expired."
                End If
    ...
    Public Function ValidateFields(ByRef pErrMsg As String) As Boolean
        ...
        Try
            ...
            If txtOldPW.Text = "" Then
                ...
                bTrue = False
            Else
                'OldPW = objUser.EncryptString(Common.Parse(txtOldPW.Text), "[+-*/]")
                OldPW = Common.EncryptString(txtOldPW.Text)
            ...
            If txtNewPW.Text <> txtConfNPW.Text Then
                pErrMsg &= "<LI type=square>New Password and Confirm New Password must be identical."
                Return False
            End If

            If (Len(txtNewPW.Text) < iPwdMin And txtNewPW.Text <> "") Or (Len(txtNewPW.Text) > iPwdMax And txtNewPW.Text <> "") Then
                pErrMsg &= "<LI type=square>Password must be " & iPwdMin & " - " & iPwdMax & " alpha-numeric."
                bTrue = False
            End If
