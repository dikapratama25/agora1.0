...
#End Region
    ...
    Private Sub cmdModify_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdModify.Click
        ...
        For Each grdItem In dtgDept.Items
            ...
            If chkSelection.Checked Then
                Me.Response.Redirect(dDispatcher.direct("Admin", "adddept.aspx", "side=" & Request.QueryString("side") & "&mode=update&index=" & dtgDept.DataKeys.Item(grdItem.ItemIndex) & "&pageid=" & strPageId & "&deptcode=" & Server.UrlEncode(grdItem.Cells(2).Text) & "&deptname=" & Server.UrlEncode(grdItem.Cells(3).Text) & "&appgrpindex=" & Server.UrlEncode(grdItem.Cells(4).Text)))
    ...
    Private Sub dtgDept_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.DataGridItemEventArgs) Handles dtgDept.ItemDataBound
        If (e.Item.ItemType = ListItemType.Item Or e.Item.ItemType = ListItemType.AlternatingItem) Then
            ...
            lnkDeptCode.NavigateUrl = "" & dDispatcher.direct("Admin", "addDept.aspx", "side=" & Request.QueryString("side") & "&mode=update&pageid=" & strPageId & "&deptcode=" & Server.UrlEncode(dv("CDM_DEPT_CODE")) & "&deptname=" & Server.UrlEncode(dv("CDM_DEPT_NAME")) & "&appgrpindex=" & dv("CDM_APPROVAL_GRP_INDEX") & "&index=" & dv("CDM_DEPT_INDEX"))
            lnkDeptCode.Text = dv("CDM_DEPT_CODE")

            'Zulham 15102018 - PAMB SST
            Dim ds As New DataSet
            Dim lblResidentGroups As Label : lblResidentGroups = e.Item.FindControl("lblResidentGroups")
            Dim lblNonResidentGroups As Label : lblNonResidentGroups = e.Item.FindControl("lblNonResidentGroups")

            ds = objDb.FillDs("Select AGM1.AGM_GRP_NAME
                                ...
                                And cdm1.cdm_dept_code = '" & dv("CDM_DEPT_CODE") & "'")
            If Not ds.Tables(0).Rows.Count = 0 Then
                For i As Integer = 0 To ds.Tables(0).Rows.Count - 1
                    If lblResidentGroups.Text = "" Then
                        lblResidentGroups.Text = ds.Tables(0).Rows(i).Item(0)
                    Else
                        lblResidentGroups.Text = lblResidentGroups.Text & "<br>" & ds.Tables(0).Rows(i).Item(0)
                    End If
                Next
            End If
            ds = objDb.FillDs("Select AGM1.AGM_GRP_NAME
                                ...
                                Left Join APPROVAL_GRP_MSTR AGM1 ON CDM1.CDM_APPROVAL_GRP_INDEX = AGM1.AGM_GRP_INDEX  
                                WHERE CDM1.CDM_COY_ID ='" & HttpContext.Current.Session("CompanyId") & "' AND CDM1.CDM_DELETED='N' 
                                And AGM1.AGM_RESIDENT IN ('N')  
                                And cdm1.cdm_dept_code = '" & dv("CDM_DEPT_CODE") & "'")
            If Not ds.Tables(0).Rows.Count = 0 Then
                For i As Integer = 0 To ds.Tables(0).Rows.Count - 1
                    If lblNonResidentGroups.Text = "" Then
                        lblNonResidentGroups.Text = ds.Tables(0).Rows(i).Item(0)
                    Else
                        lblNonResidentGroups.Text = lblNonResidentGroups.Text & "<br>" & ds.Tables(0).Rows(i).Item(0)
                    End If
                Next
            End If


            'Get IPP approval group name
            'Dim groupname As String
            'groupname = objDb.GetVal("SELECT AGM_GRP_NAME FROM APPROVAL_GRP_MSTR WHERE AGM_GRP_INDEX='" & dv("CDM_IPP_APPROVAL_GRP_INDEX") & "'")
            'If groupname <> "" Then
            '    If e.Item.Cells(5).Text <> "&nbsp;" Then
            '        e.Item.Cells(5).Text = e.Item.Cells(5).Text & " / " & groupname
            '    Else
            '        e.Item.Cells(5).Text = "- / " & groupname
            '    End If
            'Else
            '    If e.Item.Cells(5).Text <> "&nbsp;" Then
