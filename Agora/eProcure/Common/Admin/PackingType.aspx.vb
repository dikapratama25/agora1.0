...
Public Class PackingType
    ...
    Protected WithEvents dtgPack As System.Web.UI.WebControls.DataGrid
    Protected WithEvents textPackCode As System.Web.UI.WebControls.TextBox
    Protected WithEvents textPackName As System.Web.UI.WebControls.TextBox
    Protected WithEvents ddl_Select As System.Web.UI.WebControls.DropDownList
    Protected WithEvents txt_add_PackCode As System.Web.UI.WebControls.TextBox
    Protected WithEvents txt_add_PackName As System.Web.UI.WebControls.TextBox
    ...
    Protected WithEvents validate_Pack_code As System.Web.UI.WebControls.RequiredFieldValidator
    Protected WithEvents validate_Pack_name As System.Web.UI.WebControls.RequiredFieldValidator
    Dim ds As DataSet
...
#End Region
    ...
    Sub dtgPack_Page(ByVal sender As Object, ByVal e As DataGridPageChangedEventArgs) Handles dtgPack.PageIndexChanged
        dtgPack.CurrentPageIndex = e.NewPageIndex
        Bindgrid(True)
    End Sub

    Sub SortCommand_Click(ByVal sender As Object, ByVal e As DataGridSortCommandEventArgs) Handles dtgPack.SortCommand
        Grid_SortCommand(sender, e)
        dtgPack.CurrentPageIndex = 0
        Bindgrid(True)
    End Sub
    ...
    Private Function Bindgrid(Optional ByVal pSorted As Boolean = False) As String
        ...
        ds = objAdmin.getPackType(textPackCode.Text, textPackName.Text)
        ...
        If intPageRecordCnt > 0 Then
            ...
            resetDatagridPageIndex(dtgPack, dvViewPack)
            dtgPack.DataSource = dvViewPack
            dtgPack.DataBind()
            dtgPack.Visible = True
        Else
            cmdModify.Enabled = False
            cmdDelete.Enabled = False
            dtgPack.Visible = False
            Common.NetMsgbox(Me, MsgNoRecord)
        End If

        ViewState("PageCount") = dtgPack.PageCount
    End Function

    Private Sub dtgPack_ItemCreated(ByVal sender As System.Object, ByVal e As System.Web.UI.WebControls.DataGridItemEventArgs) Handles dtgPack.ItemCreated

        intPageRecordCnt = ViewState("intPageRecordCnt")
        Grid_ItemCreated(dtgPack, e)
    ...
    Private Sub dtgPack_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.DataGridItemEventArgs) Handles dtgPack.ItemDataBound
    ...
    Private Sub cmdAdd_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmdAdd.Click
        ...
        validate_Pack_code.Enabled = True
        validate_Pack_name.Enabled = True
        txt_add_PackCode.Enabled = True
        txt_add_PackCode.Text = ""
        txt_add_PackName.Text = ""
        Me.lbl_add_mod.Text = "add"
        cmd_clear.Text = "Clear"
        hidMode.Value = "a"

        For Each dgItem In dtgPack.Items
    ...
    Private Sub cmd_clear_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmd_clear.Click
        If hidMode.Value = "a" Then
            ...
            txt_add_PackName.Text = ""
        Else
            txt_add_PackCode.Text = ViewState("oldPackCode")
            txt_add_PackName.Text = ViewState("oldPackName")
        End If
    End Sub
    ...
    Sub view(ByVal selected As String)
        ...
        If selected = "add" Then
            Me.lbl_add_mod.Text = "add"
            intmsgno = objAdm.addPackType(txt_add_PackCode.Text, txt_add_PackName.Text)
        ElseIf selected = "mod" Then
            Me.lbl_add_mod.Text = "modify"
            intmsgno = objAdm.modPackType(hidIndex.Value, txt_add_PackCode.Text, txt_add_PackName.Text)
        End If
        ...
        Select Case intmsgno
            ...
            Case WheelMsgNum.Duplicate
                Common.NetMsgbox(Me, "Packing Code duplicated.", MsgBoxStyle.Information)
        End Select
    End Sub
    ...
    Private Sub cmdModify_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmdModify.Click
        ...
        validate_Pack_code.Enabled = True
        validate_Pack_name.Enabled = True
        txt_add_PackCode.Enabled = False
        Me.lbl_add_mod.Text = "Modify"
        ...
        For Each dgItem In dtgPack.Items
            chk = dgItem.FindControl("chkSelection")
            If chk.Checked Then
                txt_add_PackCode.Text = dgItem.Cells(1).Text
                txt_add_PackName.Text = dgItem.Cells(4).Text
                hidIndex.Value = dgItem.Cells(2).Text
                ViewState("oldPackCode") = dgItem.Cells(1).Text
                ViewState("oldPackName") = dgItem.Cells(4).Text
                Exit For
            End If
            i = i + 1
        Next
    ...
    Private Sub cmdDelete_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmdDelete.Click
        ...
        Dim dtNewPack As New DataTable
        Dim drPack As DataRow
        dtNewPack.Columns.Add("Pack_Code", Type.GetType("System.String")) '//product code
        dtNewPack.Columns.Add("Pack_Index", Type.GetType("System.Int32")) '//supplier id

        For Each dgItem In dtgPack.Items
            Dim chkSelection As CheckBox = dgItem.Cells(0).FindControl("chkSelection")

            If chkSelection.Checked Then
                drPack = dtNewPack.NewRow
                drPack("Pack_Code") = dgItem.Cells(1).Text
                drPack("Pack_Index") = dgItem.Cells(2).Text
                dtNewPack.Rows.Add(drPack)
            End If
        Next

        Dim objAdmin As New Admin
        intMsg = objAdmin.delPackType(dtNewPack)
        If intMsg = -99 Then
