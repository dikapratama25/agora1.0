...
Partial Public Class AccountCodeAudit
    ...
    Private Sub ExportToPDF()
        ...
        Try
            ...
            With cmd
                ...
                .CommandText = " SELECT AU_LOG_INDEX, AU_AM_ACCT_MAP_INDEX, AU_AM_F_ACCT_INDEX, AU_AM_F_BR_CODE, AU_AM_F_GL_CODE, AU_AM_F_CC, " _
                                & "AU_AM_F_CC_DESC, AU_AM_F_MAP_CODE, AU_AM_T_ACCT_INDEX, AU_AM_T_BR_CODE, AU_AM_T_GL_CODE, " _
                                & "AU_AM_T_CC, AU_AM_T_CC_DESC, AU_AM_T_MAP_CODE, AU_USER, AU_DATE, AU_ACTION " _
                                & "FROM au_account_mapping INNER JOIN user_mstr " _
                                & "ON au_account_mapping.AU_USER = user_mstr.UM_USER_ID AND user_mstr.UM_COY_ID = AU_AM_COY_ID " _
                                & "WHERE AU_AM_COY_ID=@coyID "
                ...
                If txtBranchCode.Text <> "" Then
                    .CommandText &= "AND AU_AM_F_BR_CODE LIKE '%" & txtBranchCode.Text & "%' "
                End If

                If txtBranchCodeTo.Text <> "" Then
                    .CommandText &= "AND AU_AM_T_BR_CODE LIKE '%" & txtBranchCodeTo.Text & "%' "
                End If

                If txtGLCode.Text <> "" Then
                    .CommandText &= "AND AU_AM_F_GL_CODE LIKE '%" & txtGLCode.Text & "%' "
                End If

                If txtGLCodeTo.Text <> "" Then
                    .CommandText &= "AND AU_AM_T_GL_CODE LIKE '%" & txtGLCodeTo.Text & "%' "
                End If

                If txtCostCenter.Text <> "" Then
                    .CommandText &= "AND AU_AM_F_CC LIKE '%" & txtCostCenter.Text & "%' "
                End If

                If txtCostCenterTo.Text <> "" Then
                    .CommandText &= "AND AU_AM_T_CC LIKE '%" & txtCostCenterTo.Text & "%' "
                End If

                If txtInterfaceCode.Text <> "" Then
                    .CommandText &= "AND AU_AM_F_MAP_CODE LIKE '%" & txtInterfaceCode.Text & "%' "
                End If

                If txtInterfaceCode.Text <> "" Then
                    .CommandText &= "AND AU_AM_T_MAP_CODE LIKE '%" & txtInterfaceCodeTo.Text & "%' "
                End If
            ...
            Dim rptDataSource As New Microsoft.Reporting.WebForms.ReportDataSource("DataSetAccountCodeAuditTable_DataSetAccountCodeAudit", ds.Tables(0))
            ...
            localreport.ReportPath = appPath & "Common\Report\PreviewAccountCodeAuditTrail.rdlc"
            ...
            strFileName = "AccountCodeMappingAuditTrail.pdf"
            'Return PDF
            Me.Response.Clear()
    ...
    Private Sub ExportToExcel()
        ...
        Try
            ...
            With cmd
                .Connection = conn
                .CommandType = CommandType.Text
                .CommandText = " SELECT DATE_FORMAT(AU_DATE,'%d/%m/%Y %T') AS 'Action Date Time'," _
                               & "CASE WHEN AU_ACTION = 'N' THEN 'Add' " _
                               & "ELSE " _
                               & "CASE WHEN AU_ACTION = 'D' THEN 'Delete' " _
                               & "ELSE 'Modify' END " _
                               & "END AS 'Action Type', " _
                               & "CONCAT(AU_AM_F_BR_CODE, ' : ', AU_AM_F_GL_CODE, ' : ', AU_AM_F_CC, ' ', AU_AM_F_CC_DESC, ' : ', AU_AM_F_MAP_CODE) AS 'From (Branch Code:BR GL Code: GL:Cost Center:Interface Code)', " _
                               & "CONCAT(AU_AM_T_BR_CODE, ' : ', AU_AM_T_GL_CODE, ' : ', AU_AM_T_CC, ' ',AU_AM_T_CC_DESC, ' : ', AU_AM_T_MAP_CODE) AS 'To (Branch Code:BR GL Code: GL:Cost Center:Interface Code)' " _
                               & "FROM(au_account_mapping) " _
                               & "WHERE AU_AM_COY_ID='" & Session("CompanyID") & "' "
                If txtDateFr.Text <> "" Then
                    strBeginDate = Format(CDate(Me.txtDateFr.Text), "yyyy-MM-dd" & " " & "00:00:00")
                    strStart = Format(CDate(Me.txtDateFr.Text), "ddMMMyyyy")
            ...
            strFileName = "AccountCodeAuditTrail" & strDate & ".xls"
            Dim attachment As String = "attachment;filename=" & strFileName
            Response.ClearContent()
            Response.AddHeader("Content-Disposition", attachment)
            Response.ContentType = "application/vnd.ms-excel"

            Response.Write("Account Code Audit Report")
        ...
        If strFrm <> "ItemCat" Then
            Session("w_AccountCode_tabs") = "<div class=""t_entity""><ul>" & _
                    "<li><div class=""space""></div></li>" & _
                                      "<li><a class=""t_entity_btn"" href=""" & dDispatcher.direct("Interface", "AccCode.aspx", "pageid=" & strPageId) & """><span>Account Code</span></a></li>" & _
                                      "<li><div class=""space""></div></li>" & _
                                      "<li><a class=""t_entity_btn_selected"" href=""" & dDispatcher.direct("Interface", "AccountCodeAudit.aspx", "pageid=" & strPageId) & """><span>Audit</span></a></li>" & _
                                      "<li><div class=""space""></div></li>" & _
                                      "</ul><div></div></div>"
        Else
