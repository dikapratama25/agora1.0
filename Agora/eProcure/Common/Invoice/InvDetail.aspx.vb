...
#End Region
    ...
    Protected Overrides Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If Not IsPostBack Then
            ...
            If Request(Trim("tax")) <> "" Then
                ViewState("Tax") = CInt(Request(Trim("tax")))
            Else
                ViewState("Tax") = 0
            End If
    ...
    Sub AddRowtotal() 'add total row 
        ...
        row4.Cells(4).Text = "Invoice Tax (" & ViewState("Tax") & "%)"
        row4.Cells(4).HorizontalAlign = HorizontalAlign.Right
        row4.Cells(4).Font.Bold = True

        row4.Cells(5).Text = Format(((ViewState("total") + ViewState("gst") + ViewState("ShipAmt")) / 100) * ViewState("Tax"), "#,##0.00")
        ...
        row5.Cells(4).Text = "Grand Total"
        row5.Cells(4).HorizontalAlign = HorizontalAlign.Right
        row5.Cells(4).Font.Bold = True

        row5.Cells(5).Text = Format((((ViewState("total") + ViewState("gst") + ViewState("ShipAmt")) / 100) * ViewState("Tax")) + (ViewState("total") + ViewState("gst") + ViewState("ShipAmt")), "#,##0.00")
        row5.Cells(5).HorizontalAlign = HorizontalAlign.Right
        row5.Cells(5).Font.Bold = True
    ...
    Private Sub GenerateTab()
        ...
        "<li><div class=""space""></div></li>" &
                            "<li><a class=""t_entity_btn_selected"" href=""" & dDispatcher.direct("Invoice", "InvList.aspx", "pageid=" & strPageId) & """><span>Issue Invoice</span></a></li>" &
                                 "<li><div class=""space""></div></li>" &
                            "<li><a class=""t_entity_btn"" href=""" & dDispatcher.direct("Invoice", "invoiceView.aspx", "pageid=" & strPageId) & """><span>Invoice Listing</span></a></li>" &
                                 "<li><div class=""space""></div></li>" &
                            "</ul><div></div></div>"
