...
Public Class POApprovalSetup
    Inherits AgoraLegacy.AppBaseClass
    Dim dDispatcher As New AgoraLegacy.dispatcher
    Dim objDB As New EAD.DBCom
    Dim blnFFPO As Boolean
...
#Region " Web Form Designer Generated Code "
    ...
    'Jules 2018.10.19 - U00010
    Protected WithEvents lblRequesterID As System.Web.UI.WebControls.Label
    Protected WithEvents lblRequesterName As System.Web.UI.WebControls.Label
    Protected WithEvents lblPRCostCentre As System.Web.UI.WebControls.Label
    Protected WithEvents lblPRGApprovalGrp As System.Web.UI.WebControls.Label
    Protected WithEvents trPRDetail1 As System.Web.UI.HtmlControls.HtmlTableRow
    Protected WithEvents trPRDetail2 As System.Web.UI.HtmlControls.HtmlTableRow
    Protected WithEvents trPRDetail3 As System.Web.UI.HtmlControls.HtmlTableRow
    Protected WithEvents trPRDetail4 As System.Web.UI.HtmlControls.HtmlTableRow
    'End modification.
...
#End Region
    ...
    Protected Overrides Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If Not IsPostBack Then
            ...
            Dim dsPR, dsBCM As New DataSet
            dsPR = objPR.getPRDetailsforPOApprovalSetup(ViewState("poid"))
            If dsPR IsNot Nothing Then
                If dsPR.Tables(0).Rows.Count > 0 Then
                    lblRequesterID.Text = dsPR.Tables(0).Rows(0)("PRM_BUYER_ID")
                    lblRequesterName.Text = dsPR.Tables(0).Rows(0)("PRM_REQ_NAME")
                    lblPRGApprovalGrp.Text = dsPR.Tables(0).Rows(0)("AGM_GRP_NAME")

                    dsBCM = objBudget.getBCMListByUserNew(dsPR.Tables(0).Rows(0)("PRM_BUYER_ID"))
                    If dsBCM IsNot Nothing Then
                        If dsBCM.Tables(0).Rows.Count > 0 Then
                            lblPRCostCentre.Text = dsBCM.Tables(0).Rows(0)("Acct_List")
                        End If
                    End If
                    trPRDetail1.Visible = True
                    trPRDetail2.Visible = True
                    trPRDetail3.Visible = True
                    trPRDetail4.Visible = True
                Else
                    trPRDetail1.Visible = False
                    trPRDetail2.Visible = False
                    trPRDetail3.Visible = False
                    trPRDetail4.Visible = False
                End If
            End If
            'End modification.
            ...
            If dsApprovalList.Tables(0).Rows.Count > 0 Then
                ...
                ' Add ---Select--- 'Jules uncommented 2018.08.14
                lstItem.Value = ""
                lstItem.Text = "---Select---"
                ...
                'Jules 2018.08.16 - Even if there's only 1 workflow, default to Select.
                ...
                If ViewState("prindex") <> Nothing Or ViewState("prindex") <> "" Then
                    Dim strAGM_Name As String = objDB.GetVal("SELECT IFNULL(AGM_GRP_NAME,'') AS AGM_GRP_INDEX FROM APPROVAL_GRP_MSTR  WHERE AGM_TYPE = 'PR' AND AGM_COY_ID = '" & HttpContext.Current.Session("CompanyId") & "' AND AGM_GRP_INDEX = (SELECT DISTINCT PRA_APPROVAL_GRP_INDEX FROM PR_APPROVAL WHERE PRA_PR_INDEX IN (" & ViewState("prindex") & ") AND PRA_FOR = 'PR') LIMIT 1")
                    If strAGM_Name <> "" Then
        ...
        If Not _isFFPO = "" Then
            If _isFFPO = "Y" Then
                ...
                If Not Request.QueryString("Frm") Is Nothing Then
                    ...
                    ElseIf Request.QueryString("Frm").ToUpper.Contains("FFPO") Then
                        Session("urlreferer") = "RaiseFFPO"
                    Else
    ...
    Private Sub dtgAO_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.DataGridItemEventArgs) Handles dtgAO.ItemDataBound
        If e.Item.ItemType = ListItemType.AlternatingItem Or e.Item.ItemType = ListItemType.Item Then
            ...
            Select Case ViewState("ApprovalType")
                ...
                Case "B+C"
                    'Jules 2018.11.22 - To separate Buyer Approval flow from Procurement Approval flow.
                    'Michelle (10/8/2007) - should be 'Approval' if PR cost is the same as the limit
                    If dv("AOSource") = "AO" Then
                        If CDbl(ViewState("pocost")) <= CDbl(dv("UM_APP_LIMIT")) Then
                            If ViewState("blnCutPO") = True Then
                                lblType.Text = "None"
                            Else
                        ...
                        Else
                            lblType.Text = "Endorsement"
                        End If
                    ElseIf dv("AOSource") = "PAO" Then
                        If CDbl(ViewState("pocost")) <= CDbl(dv("UM_APP_LIMIT")) Then
                            If ViewState("blnCutPAO") = True Then
                                lblType.Text = "None"
                            Else
                                lblType.Text = "Approval"
                                ViewState("blnCutPAO") = True
                            End If
                        Else
                            lblType.Text = "Endorsement"
                        End If
                    ...
                    'Jules 2018.11.22 - Commented.
                    'Zulham 03082018 - PAMB
                    'Dim isPAO As String = ""
                    'isPAO = objDB.GetVal("SELECT 'Y' FROM approval_grp_pao WHERE AGPAO_AO = '" & dv("AGA_AO") & "'")
                    'If isPAO.Trim = "Y" Then
                    '    If CDbl(ViewState("pocost")) <= CDbl(dv("UM_APP_LIMIT")) Then
                    '        If ViewState("blnCutPAO") = True Then
                    '            lblType.Text = "None"
                    '        Else
            ...
            Select Case intMsg
                ...
                Case WheelMsgNum.Save
                    ...
                    ElseIf Session("urlreferer") = "RaiseFFPO" Then
                        Common.NetMsgbox(Me, "Purchase Order Number " & ViewState("poid") & " has been submitted." & """ & vbCrLf & """ & strConfirm & "", dDispatcher.direct("PO", "RaiseFFPO.aspx", "mode=po&type=new&pageid=" & strPageId))
    ...
    Private Sub GenerateTab()
        ...
        If showFFPO.ToString.ToUpper = "Y" Then
            'Michelle (20/2/2014) _ Issue 2694
            ' If Not Session("urlreferer") = "BuyerCatalogueSearch" Then
            If Not Session("urlreferer") = "BuyerCatalogueSearch" And blnFFPO Then
                Session("w_AddPOAppr_tabs") = "<div class=""t_entity""><ul>" &
            ...
            "<li><a class=""t_entity_btn"" href=""" & dDispatcher.direct("PO", "POViewB2Cancel.aspx", "type=Listing&pageid=" & strPageId) & """><span>Purchase Order Cancellation</span></a></li>" &
            "<li><div class=""space""></div></li>" &
            "<li><a class=""t_entity_btn"" href=""" & dDispatcher.direct("PO", "RAISEFFPO.aspx", "type=Listing&pageid=" & strPageId) & """><span>Free Form Purchase Order</span></a></li>" &
            "<li><div class=""space""></div></li>" &
            "</ul><div></div></div>"
            End If
        Else
