<%@ Page Language="vb" AutoEventWireup="false" Codebehind="GLCodeSearchP2P.aspx.vb" Inherits="eProcure.GLCodeSearchP2P" %>
...
<HTML>
	<HEAD>
		...
		<!--		
			...
			function selectOne()
			{
                ...
                var r = (eval("window.opener.document.Form1." + document.Form1.hidGLCode.value));
                r.value = document.Form1.hidGLCodeValue.value;
            ...
            {                 
                ...
                var chkStatus;
                re = new RegExp('$')	
                //alert('oform.elements.length: ' + oform.elements.length);
				for (var i=0;i<oform.elements.length;i++)
				{
				    var foo = "$";
					var e = oform.elements[i];
                    ...
                    {					                                                
                        //if (sEvents.substring(sEvents.lastIndexOf("$") + 1) == "cmdGLCode") {
                        //    //chkStatus = (eval("window.opener.document.Form1." + sEvents));
                        //    sStatus = sEvents.substring(0, sEvents.indexOf("$")) + "_" + sEvents.substring(sEvents.indexOf("$") + 1, sEvents.lastIndexOf("$")) + "_cmdGLCode";
                        ...
                        if (sEvents.substring(sEvents.lastIndexOf("$") + 1) == "hidGLCodeStatus") {
                            //chkStatus = (eval("window.opener.document.Form1." + sEvents));
                            sStatus = sEvents.substring(0, sEvents.indexOf("$")) + "_" + sEvents.substring(sEvents.indexOf("$") + 1, sEvents.lastIndexOf("$")) + "_hidGLCodeStatus";
                            //alert('sStatus: ' + sStatus);
                            //alert('sStatus.value= ' + window.opener.document.getElementById(sStatus).value);
                            if (window.opener.document.getElementById(sStatus).value == 'N') {
                                chkStatus = false;
                            }
                            else {
                                chkStatus = true;
                        ...
                        if (sEvents.substring(sEvents.lastIndexOf("$") + 1) == "hidGLCode" && chkStatus == true)
                        {					
					            var lmtDesc = document.Form1.hidItem.value;
					            var sGLCode = sEvents.substring(0, sEvents.indexOf("$")) + "_" + sEvents.substring(sEvents.indexOf("$")+1 , sEvents.lastIndexOf("$")) + "_txtGLCode";                            
                                //alert('sGLCode: ' + sGLCode);                                
                                window.opener.document.getElementById(sGLCode).innerHTML = lmtDesc; //lmtDesc.substring(0,9);
					            window.opener.document.getElementById(sGLCode).title = lmtDesc;  
			...
			function Chk(val,valdesc)
            {
                //Jules 2018.11.05 - Swapped GL Code display: "GL Description(GL Code)
                //document.Form1.hidItem.value = '(' + val + ')' + valdesc;
                document.Form1.hidItem.value = valdesc + ' (' + val + ')';
                document.Form1.hidGLCodeValue.value = val;
			}
	...
	<body MS_POSITIONING="GridLayout">
		<form id="Form1" method="post" runat="server">
			<TABLE class="alltable" id="Table1" cellSpacing="0" cellPadding="1" width="100%">
				<TR>
				...
				<TR>
					<TD colspan="3"><asp:datagrid id="dtgGLCode" runat="server" AutoGenerateColumns="False" OnSortCommand="SortCommandValues_Click"
							AllowSorting="True">
							...
							<Columns>
                                ...
                                <asp:BoundColumn DataField="CBG_B_GL_DESC" SortExpression="CBG_B_GL_DESC" HeaderText="Description">
									<HeaderStyle Width="70%"></HeaderStyle>
								</asp:BoundColumn>	
                                <asp:BoundColumn DataField="CBG_B_GL_CODE" SortExpression="CBG_B_GL_CODE" HeaderText="Code">
				...
				<TR id="trP" runat="server">
					<TD colspan="3" style="WIDTH: 211px"><INPUT class="button" id="cmdSelect" disabled onclick="Select();" type="button" value="Save"
						...
						<INPUT id="hidID" style="WIDTH: 35px; HEIGHT: 22px" type="hidden" size="1" name="hidID"
							...
							runat="server"><INPUT id="hidGLCode" style="WIDTH: 40px; HEIGHT: 22px" type="hidden" size="1" name="hidGLCode"
							runat="server"><INPUT id="hidGLCodeValue" style="WIDTH: 40px; HEIGHT: 22px" type="hidden" size="1" name="hidGLCodeValue"
							runat="server"></TD>
				</TR>
				<TR>
