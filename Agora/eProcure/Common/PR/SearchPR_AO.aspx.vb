...
#End Region
    ...
    Private Function Bindgrid(Optional ByVal pSorted As Boolean = False) As String
        ...
        ds = objPR.getPRListForApproval(txtPRNo.Text, txtVendor.Text, txtDateFr.Text, txtDateTo.Text, strReliefOn, , "", "", strPRType, "", "IncludeHold")
    ...
    Private Sub dtgPRList_ItemDataBound(ByVal sender As System.Object, ByVal e As System.Web.UI.WebControls.DataGridItemEventArgs) Handles dtgPRList.ItemDataBound
        If (e.Item.ItemType = ListItemType.Item Or e.Item.ItemType = ListItemType.AlternatingItem) Then
            ...
            Dim strSName, strSNameID, strSName2, strSNameID2 As String
            Dim objDb As New EAD.DBCom
            If e.Item.Cells(EnumPR.icVendor).Text <> "Multiple Vendors" Then
                'e.Item.Cells(EnumPR.icVendor).Text = "<a href='../RFQ/RFQ_VendorDetail.aspx?pageid=" & strPageId & "&v_com_id=" & Common.parseNull(dv("PRM_S_Coy_ID")) & "'>" & e.Item.Cells(EnumPR.icVendor).Text & "</a>"
            ...
            Else
                ...
                'Else
                '    strSNameID2 = objDb.GetVal("SELECT IFNULL(PRD_S_COY_ID, '') PRD_S_COY_ID FROM PR_DETAILS WHERE PRD_COY_ID = '" & HttpContext.Current.Session("CompanyId") & "' AND PRD_PR_NO = '" & Common.parseNull(dv("PRM_PR_No")) & "' AND PRD_VENDOR_ITEM_CODE <> '' AND PRD_S_COY_ID <> '&nbsp;' HAVING COUNT(DISTINCT IFNULL(PRD_S_COY_ID, '')) = 1")
                '    strSName2 = objDb.GetVal("SELECT IFNULL((SELECT CM_COY_NAME FROM COMPANY_MSTR WHERE CM_COY_ID = PR_DETAILS.PRD_S_COY_ID), '') PRD_S_COY_ID FROM PR_DETAILS WHERE PRD_COY_ID = '" & HttpContext.Current.Session("CompanyId") & "' AND PRD_PR_NO = '" & Common.parseNull(dv("PRM_PR_No")) & "' AND PRD_VENDOR_ITEM_CODE <> '' AND PRD_S_COY_ID <> '&nbsp;' HAVING COUNT(DISTINCT IFNULL(PRD_S_COY_ID, '')) = 1")

                '    If strSNameID2 <> "" Then
                '        e.Item.Cells(EnumPR.icVendor).Text = "<A href=""" & dDispatcher.direct("RFQ", "RFQ_VendorDetail.aspx", "Frm=RFQ_Outstg_List&pageid=" & strPageId & "&RFQ_Num=" & Server.UrlEncode(Common.parseNull(dv("PRM_PR_No"))) & "&v_com_id=" & strSNameID2) & """ ><font color=#0000ff>" & strSName2 & "</font></A>"
                '    Else
                '        'e.Item.Cells(EnumPR.icVendor).Text = "<a href='../PR/Multi_VendorDetails.aspx?caller=AO&pageid=" & strPageId & "&PRNum=" & Common.parseNull(dv("PRM_PR_NO")) & "'>" & e.Item.Cells(EnumPR.icVendor).Text & "</A>"
            ...
            If Not IsDBNull(dv("PRM_RFQ_INDEX")) AndAlso CStr(dv("PRM_RFQ_INDEX")) <> "" Then
                ...
                lnkRFQ.NavigateUrl = dDispatcher.direct("RFQ", "RFQComSummary.aspx", "Frm=SearchPR_AO&pageid=" & strPageId & "&side=other&RFQ_Num=" & strRFQNo & "&RFQ_ID=" & dv("PRM_RFQ_INDEX") & "&RFQ_Name=" & Server.UrlEncode(strRFQName))
                lnkRFQ.ToolTip = "Click here to view quotation comparison"
                e.Item.Cells(EnumPR.icPRNoLink).Controls.Add(lnkRFQ)
            End If
            ...
            e.Item.Cells(EnumPR.icStatus).Text = dv("STATUS_DESC")

            If dv("STATUS_DESC") = "Held By" Then
                e.Item.Cells(EnumPR.icStatus).Text = "Held By " & Common.parseNull(dv("NAME"))
            End If
        End If
    End Sub
