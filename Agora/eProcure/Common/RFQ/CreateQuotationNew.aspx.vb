...
Public Class CreateQuotationNew
    Inherits AgoraLegacy.AppBaseClass
    Dim dDispatcher As New AgoraLegacy.dispatcher
    'Dim blnTrue As Boolean = False 'Delivery Term no exist
...
#End Region
    ...
    Protected Overrides Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        'If objDb.Exist("SELECT '*' FROM COMPANY_DELIVERY_TERM WHERE CDT_COY_ID = '" & Request.QueryString("bcomid") & "'") Then
        '    blnTrue = True
        'End If
        ...
        If Not Page.IsPostBack Or hidPostBack.Value = "1" Then
            ...
            ViewState("strGstRegNo") = objGst.chkGST()
            ViewState("GstQuo") = objGst.chkGSTCOD()
            hidGst.Value = ViewState("GstQuo")
            objGst = Nothing
            ...
            Session("rfd_id") = Request(Trim("RFQ_ID"))
            Dim RFQ_NO As String = Request(Trim("RFQ_NO"))
            ...
            'Dim strTemp As String = "SELECT '*' FROM RFQ_REPLIES_DETAIL_TEMP WHERE RRDT_RFQ_ID = '" & Request(Trim("RFQ_ID")) & "' AND RRDT_ITEM_TYPE = 'ST'"

            'Check for Delivery Term setup
            If objDb.Exist("SELECT '*' FROM RFQ_MSTR WHERE RM_RFQ_ID = '" & Request(Trim("RFQ_ID")) & "' AND (RM_DEL_CODE <> '' AND RM_DEL_CODE IS NOT NULL)") Then
                Session("blnTrue") = True
                'If objDb.Exist(strTemp) Then
                '    Session("blnTrue") = True
                'Else
            ...
            If Session("blnTrue") = True Then
                ...
                Common.SelDdl(objval.cur_code, ddl_curr, True, True)
                Session("del_term") = objval.del_code
            End If
        ...
        Me.cmdPreview.Attributes.Add("onclick", "PopWindow('" & dDispatcher.direct("Report", "PreviewRFQ.aspx", "VendorRequired=T&BCoyID=" & ViewState("RM_Coy_ID") & "&SCoyID=" & Session("CompanyId") & "&rfqid=" & ViewState("rfq_id") & "&RFQ_Num=" & ViewState("rfq_no")) & "')")
    End Sub
    ...
    Private Function Bindgrid(Optional ByVal pPage As Integer = -1, Optional ByVal pSorted As Boolean = False) As String
        ...
        If Session("blnTrue") = True Then
            dg_viewitem.Columns(ItemEnum.icDT).Visible = True
        End If
        ...
        If ViewState("GstQuo") = True Then
            dg_viewitem.Columns(ItemEnum.icTaxAmt).Visible = True
        Else
            dg_viewitem.Columns(ItemEnum.icTaxAmt).Visible = False
        End If
    ...
    Private Sub dg_viewitem_ItemCommand(ByVal source As Object, ByVal e As System.Web.UI.WebControls.DataGridCommandEventArgs) Handles dg_viewitem.ItemCommand
        ...
        If (e.CommandSource).CommandName = "" Then
            ...
            txtQty = e.Item.FindControl("txt_temp")
            strName = "qty=" & txtQty.Text & "&itemline=" & Server.UrlEncode(e.Item.Cells(ItemEnum.icLineNo).Text) & "&itemname=" & Server.UrlEncode(e.Item.Cells(ItemEnum.icItemName).Text) & "&mode=" & Server.UrlEncode(strmode) & "&rfqid=" & Session("rfd_id") & ""

            strscript.Append("<script language=""javascript"">")
            strFileName = dDispatcher.direct("RFQ", "UnitPriceQuotation.aspx", strName)
            strFileName = Server.UrlEncode(strFileName)
            strscript.Append("ShowDialog('" & dDispatcher.direct("RFQ", "Dialog.aspx", "page=" & strFileName) & "','400px');")
    ...
    Private Sub dg_viewitem_ItemCreated(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.DataGridItemEventArgs) Handles dg_viewitem.ItemCreated
        ...
        If e.Item.ItemType = ListItemType.Header Then
            If ViewState("GstQuo") = True Then
                e.Item.Cells(7).Text = "SST Rate"
                e.Item.Cells(8).Text = "SST Amount"
            Else
                e.Item.Cells(7).Text = "Tax"
    ...
    Private Function validateDatagrid(ByRef strMsg As String) As Boolean
        ...
        Dim txtQ As TextBox
        Dim ddl_dt As DropDownList

        strMsg = "<ul type='disc'>"

        If Session("blnTrue") = True And ddl_curr.SelectedIndex = 0 Then
            strMsg &= "<li>Currency " & objGlobal.GetErrorMessage("00001") & "<ul type='disc'></ul></li>"
        ...
        For Each dgItem In dg_viewitem.Items
            ddl_dt = dgItem.FindControl("ddl_dt")

            If Session("blnTrue") = True Then
                If ddl_dt.SelectedValue = "" Then
                    strMsg &= "<li>Delivery Term " & objGlobal.GetErrorMessage("00001") & "<ul type='disc'></ul></li>"
                    validateDatagrid = False

                ElseIf ddl_dt.SelectedValue <> "" And ddl_curr.SelectedIndex > 0 Then
                    If ddl_curr.SelectedValue <> "MYR" And ddl_dt.SelectedValue = "3233" Then
                        strMsg &= "<li>" & objGlobal.GetErrorMessage("00364") & "<ul type='disc'></ul></li>"
                        validateDatagrid = False
                        Exit For
                    End If
                End If
    ...
    Private Function validateUnitPrice() As Boolean
        ...
        '        Common.NetMsgbox(Me, "Please set unit price at least one item.", MsgBoxStyle.Information)
    ...
    Private Sub dg_viewitem_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.DataGridItemEventArgs) Handles dg_viewitem.ItemDataBound
        Dim objrfq As New RFQ
        Dim objGlo_Ext As New AppGlobals_Ext
        If (e.Item.ItemType = ListItemType.Item Or e.Item.ItemType = ListItemType.AlternatingItem) Then
            ...
            If ViewState("GstQuo") = True Then
                If ViewState("strGstRegNo") = "" Then
                    ...
                    lstItem.Value = "N/A"
                    lstItem.Text = "N/A"
                    ddl_tax.Items.Insert(0, lstItem)
                    ddl_tax.Enabled = False
                Else
                    Dim strDefaultRate As String
                    If IsDBNull(dv("RRDT_GST_RATE")) Then
                        ...
                        '    ddl_tax.SelectedValue = strDefaultRate
                        'End If
                    Else
                        strDefaultRate = dv("RRDT_GST_RATE")
                        'Common.SelDdl(Common.parseNull(dv("RRDT_GST_RATE")), ddl_tax)
                    End If

                    objGlobal.FillGST(ddl_tax, False, ViewState("RM_Coy_ID"))
                    Common.SelDdl(strDefaultRate, ddl_tax)
                End If
            Else
            ...
            e.Item.Cells(ItemEnum.icItemName).Text = Common.parseNull(dv("RRDT_Product_Desc"))
            e.Item.Cells(ItemEnum.icUom).Text = Common.parseNull(dv("RRDT_UOM"))
            'e.Item.Cells(3).Text = Common.parseNull(dv("RRDT_Quantity"))
            txt_temp.Text = Common.parseNull(dv("RRDT_Quantity"))
            e.Item.Cells(ItemEnum.icLineNo).Text = Common.parseNull(dv("RRDT_Line_No"))
            e.Item.Cells(ItemEnum.icProID).Text = Common.parseNull(dv("RRDT_Product_ID"))
            e.Item.Cells(ItemEnum.icItemType).Text = Common.parseNull(dv("RRDT_Item_Type"))
            ...
            If Session("blnTrue") = True Then
                ...
                '    Dim aryPrice2 As New ArrayList
                '    Dim aryPrice1 As New ArrayList
                '    Dim i As Integer

                '    aryPrice1 = Session("aryPrice")

                '    For i = 0 To aryPrice1.Count - 1
                '        If aryPrice1(i)(3) = Common.parseNull(dv("RRDT_Line_No")) Then
                '            If aryPrice1(i)(0) <> "" And aryPrice1(i)(1) <> "" Then
                '                aryPrice2.Add(New String() {aryPrice1(i)(0), aryPrice1(i)(1)})
                '            End If

                '        End If
                '    Next

                '    If aryPrice2.Count > 0 Then
                '        For i = 0 To aryPrice2.Count - 1
                '            If CInt(Common.parseNull(dv("RRDT_Quantity"))) >= CDbl(aryPrice2(i)(0)) Then
                '                txt_price.Text = aryPrice2(i)(1)
                '                'hidPrice.Value = aryPrice2(i)(1)
                '            Else
                '                Exit For
                '            End If
                '        Next
                ...
                'Delivery Term
                Dim ddl_dt As New DropDownList
                ddl_dt = e.Item.FindControl("ddl_dt")
                objGlo_Ext.FillDelTerm(ddl_dt, Request.QueryString("bcomid"))
                If Session("edit") = "1" Then
                    Common.SelDdl(Common.parseNull(dv("RRDT_DEL_CODE")), ddl_dt, True, True)
                Else
                    Common.SelDdl(Session("del_term"), ddl_dt, True, True)
                End If

                If ddl_dt IsNot Nothing Then
                    For Each li As ListItem In ddl_dt.Items
                        li.Attributes("title") = li.Text
                    Next
                End If

                'ddl_dt.Items(0).Attributes.Add("title", "Tool Tip")
                'If Session("edit") = "1" Then
                '    If dv("RRDT_ITEM_TYPE") = "ST" Then
                '        Common.SelDdl(Common.parseNull(dv("RRDT_DEL_CODE")), ddl_dt, True, True)
                '    Else
                '        Common.SelDdl("", ddl_dt, True, True)
                '    End If
                'Else
                '    If dv("RRDT_ITEM_TYPE") = "ST" Then
                '        Common.SelDdl(Session("del_term"), ddl_dt, True, True)
                ...
                'Unit Price
                Dim btn_price As Button
                btn_price = e.Item.FindControl("btn_price")

                If Common.parseNull(dv("RRDT_ITEM_TYPE")) = "ST" Then
                    btn_price.Visible = True
                    txt_price.Style("display") = "none"
                    rev_price.Enabled = False
                Else
                    btn_price.Visible = False
                    txt_price.Style("display") = ""
                    rev_price.Enabled = True
                    'ddl_dt.Enabled = False
                End If
            ...
            Dim txt_TaxAmt As TextBox
            txt_TaxAmt = e.Item.FindControl("txt_TaxAmt")
            ...
            txt_price.Attributes.Add("onfocus", "return focusControl('" &
                            txt_temp.ClientID & "', '" & txt_price.ClientID & "', '" & txt_Amount.ClientID & "', '" & ddl_tax.ClientID & "', '" & txt_TaxAmt.ClientID & "');")

            txt_price.Attributes.Add("onblur", "return calculateTotal('" &
                            txt_temp.ClientID & "', '" & txt_price.ClientID & "', '" & txt_Amount.ClientID & "', '" & ddl_tax.ClientID & "', '" & txt_TaxAmt.ClientID & "');")

            ddl_tax.Attributes.Add("onchange", "return calculateTotal('" &
                            txt_temp.ClientID & "', '" & txt_price.ClientID & "', '" & txt_Amount.ClientID & "', '" & ddl_tax.ClientID & "', '" & txt_TaxAmt.ClientID & "');")
    ...
    Sub save()
        ...
        For Each dgItem In dg_viewitem.Items
            ...
            objval.v_itemCode = objDB.Get1Column("PRODUCT_MSTR", "PM_VENDOR_ITEM_CODE", " WHERE PM_PRODUCT_CODE = '" & dgItem.Cells(ItemEnum.icProID).Text & "' ")
            objval.lineno = dgItem.Cells(ItemEnum.icLineNo).Text
            objval.product_ID = dgItem.Cells(ItemEnum.icProID).Text
            objval.item_desc = dgItem.Cells(ItemEnum.icItemName).Text
            objval.uom = dgItem.Cells(ItemEnum.icUom).Text
            ...
            If ViewState("GstQuo") = False Then
            ...
            Else
                objval.Tax = CType(dgItem.FindControl("ddl_tax"), DropDownList).SelectedItem.Value
                If CType(dgItem.FindControl("ddl_tax"), DropDownList).SelectedItem.Value = "N/A" Then
                    objval.gst_desc = "N/A"
                    objval.gst = 0
                Else
                    objval.gst_desc = objGst.getGstDesc(CType(dgItem.FindControl("ddl_tax"), DropDownList).SelectedItem.Value)
                    objGst.getGSTInfobyRate(objval.Tax, strPerc, strTaxId)
                    strPerc = IIf(strPerc = "", "0", strPerc)
                    objval.gst = strPerc
                End If
                'If CType(dgItem.FindControl("ddl_tax"), DropDownList).SelectedItem.Value <> "" Then
                '    objGst.getGSTInfobyRate(CType(dgItem.FindControl("ddl_tax"), DropDownList).SelectedItem.Value, strPerc, strTaxId)
                '    objval.Tax = strTaxId
                'Else
                '    objval.Tax = "0"
                'End If
            ...
            If Session("blnTrue") = True Then
                objval.del_code = CType(dgItem.FindControl("ddl_dt"), DropDownList).SelectedItem.Value

                strSQL = objrfq.add_2repliedTemp(objval, True)
            Else
    ...
    Protected Sub cmd_update_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmd_update.Click
        ...
        If Page.IsValid And validateDatagrid(strMsg) And validateUnitPrice() And validateInputs() And validateSSTRates() Then
            save()
            ...
            If ViewState("check") = "1" Then
                ...
                If Session("blnTrue") = True Then
                    objval.cur_code = ddl_curr.SelectedValue
                Else
                ...
                If Session("blnTrue") = True Then
                    comfirmStatus = objrfq.add_mstrreplied(objval, Session("edit"), Quotation_num, Session("aryPrice"))
                Else
                    comfirmStatus = objrfq.add_mstrreplied(objval, Session("edit"), Quotation_num)
                End If
                ...
                If comfirmStatus = "4" Then
                    ...
                    End If
                    Me.cmdView.Attributes.Add("onclick", "PopWindow('" & dDispatcher.direct("Report", "PreviewQuotation.aspx", "SCoyID=" & Session("CompanyId") & "&BCoyID=" & ViewState("RM_Coy_ID") & "&quo_no=" & Quotation_num & "&rfqid=" & ViewState("rfq_id") & "&rfq_no=" & ViewState("rfq_no")) & "')")
    ...
    Private Sub btnhidden2_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnhidden2.Click
        ...
        If Not Session("aryPrice") Is Nothing Then
            For Each dgItem In dg_viewitem.Items
                ...
                For i = 0 To aryPrice1.Count - 1
                    If aryPrice1(i)(3) = dgItem.Cells(ItemEnum.icLineNo).Text Then
                        If aryPrice1(i)(0) <> "" And aryPrice1(i)(1) <> "" Then
                            aryPrice2.Add(New String() {aryPrice1(i)(0), aryPrice1(i)(1)})
                        End If
                ...
                If aryPrice2.Count > 0 Then
                    For i = 0 To aryPrice2.Count - 1
                        If CDec(CType(dgItem.FindControl("txt_temp"), TextBox).Text) >= CDec(aryPrice2(i)(0)) Then
                            CType(dgItem.FindControl("txt_price"), TextBox).Text = aryPrice2(i)(1)
                            'hidPrice.Value = aryPrice2(i)(1)
                        Else
                            Exit For
                        End If
    ...
    Private Function validateSSTRates() As Boolean
        ...
        If ViewState("strGstRegNo") <> "" Then
            ...
            For Each dgItem In dg_viewitem.Items
                ...
                Dim ddl_tax As DropDownList
                ddl_tax = dgItem.FindControl("ddl_tax")

                If strGSTRate <> "" AndAlso ddl_tax.SelectedValue <> strGSTRate Then
                    If ddl_tax.SelectedValue = "ST6" Then
                        If strGSTRate = "ST5" OrElse strGSTRate = "ST10" Then
                            Common.NetMsgbox(Me, "Sales Tax item(s) and Service Tax item(s) cannot be combined in one document.", MsgBoxStyle.Information)
                            Return False
                        End If
                        blnSST = True
                    ElseIf ddl_tax.SelectedValue = "ST5" OrElse ddl_tax.SelectedValue = "ST10" Then
                    ...
                    End If
                End If
                strGSTRate = ddl_tax.SelectedValue
            Next
        End If
    End Function
End Class