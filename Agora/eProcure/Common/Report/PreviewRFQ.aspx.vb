...
Public Class PreviewRFQ
    ...
    Private Sub previewRFQ()
        ...
        If objDb.Exist("SELECT '*' FROM RFQ_MSTR WHERE RM_RFQ_No = '" & Request.QueryString("RFQ_Num") & "' AND RM_Coy_ID = '" & Request.QueryString("BCoyID") & "' AND (RM_DEL_CODE <> '' AND RM_DEL_CODE IS NOT NULL)") Then
            blnSEH = True
        End If
        ...
        Try
            ...
            With cmd
                ...
                .CommandText = "SELECT RFQ_MSTR.RM_RFQ_ID, RFQ_MSTR.RM_Coy_ID, RFQ_MSTR.RM_RFQ_No, RFQ_MSTR.RM_RFQ_Name, " _
                            ...
                            & "CASE WHEN RFQ_DETAIL.RD_Item_Type = 'SP' THEN 'Spot' WHEN RFQ_DETAIL.RD_Item_Type = 'ST' THEN 'Stock' " _
                            & "WHEN RFQ_DETAIL.RD_Item_Type = 'MI' THEN 'MRO' END AS RD_Item_Type, " _
                            ...
                            & "(SELECT   CONCAT(CDT_DEL_CODE, ' (', CDT_DEL_NAME, ')') " _
                            & "FROM   COMPANY_DELIVERY_TERM " _
                            & "WHERE CDT_COY_ID = RFQ_MSTR.RM_Coy_ID AND CDT_DEL_CODE = RFQ_MSTR.RM_DEL_CODE) AS DeliveryTerm, " _
                            & "(SELECT   CODE_DESC " _
            ...
            If blnSEH = True Then
                localreport.ReportPath = dispatcher.direct("Report", "PreviewRFQ-FTN2-SEH.rdlc", "Report") ' appPath & "RFQ\PreviewRFQ-FTN2.rdlc"
            Else
                localreport.ReportPath = dispatcher.direct("Report", "PreviewRFQ-FTN2.rdlc", "Report") ' appPath & "RFQ\PreviewRFQ-FTN2.rdlc"
            End If
