...
Public Class ReportFormatN2
    ...
    Private Sub ExportToPDF()
        ...
        Try
            ...
            With cmd
                ...
                Else
                    .CommandText = "SELECT   PO_MSTR.POM_PO_INDEX, PO_MSTR.POM_PO_NO, PO_MSTR.POM_B_COY_ID, PO_MSTR.POM_BUYER_ID, " _
                            ...
                            & "AND POM_PO_DATE >=@prmStartDate AND POM_PO_DATE <=@prmEndDate " _
                            & "AND STATUS_TYPE='PO' AND STATUS_NO IN (1,2,3,6,9) " _
                            & "ORDER BY POM_PO_DATE desc"
                End If
            End With
            ...
            If ViewState("ReportType") = "Buyer" Then
                localreport.ReportPath = dispatcher.direct("Report", "PODetails_pdf.rdlc", "Report") ' Server.MapPath("PODetails_pdf.rdlc")  'appPath & "Report\PODetails_pdf.rdlc"
                'localreport.ReportPath = appPath & "Report\PODetails_xls.rdlc"
            Else
                localreport.ReportPath = dispatcher.direct("Report", "PODetailsVendor_pdf.rdlc", "Report") ' Server.MapPath("PODetails_pdf.rdlc")  'appPath & "Report\PODetails_pdf.rdlc"
            End If
            ...
            With cmd
                ...
                If ViewState("ReportType") = "Buyer" Then
                    .CommandText = "SELECT PO_MSTR.POM_PO_NO AS 'PO Number', DATE_FORMAT(po_mstr.POM_PO_DATE,'%d/%m/%Y') AS 'PO Date',po_mstr.POM_S_COY_NAME AS 'Vendor Name'," _
                            ...
                            & "CT_NAME AS 'Commodity Type',b.CDM_DEPT_NAME AS 'Cost Center',PO_DETAILS.POD_B_CATEGORY_CODE AS 'Category Name', PO_DETAILS.POD_B_GL_CODE AS 'GL Code', " _
                            ...
                            & "LEFT OUTER JOIN account_mstr ON account_mstr.AM_ACCT_INDEX=po_details.POD_ACCT_INDEX  " _
                            & "LEFT OUTER JOIN company_dept_mstr b ON account_mstr.AM_DEPT_INDEX = b.CDM_DEPT_INDEX  " _
                            & "WHERE (PO_MSTR.POM_B_COY_ID =  '" & Session("CompanyID") & "') AND POM_PO_DATE >='" & strBeginDate & "' " _
                ...
                Else
                    .CommandText = "SELECT PO_MSTR.POM_PO_NO AS 'PO Number', DATE_FORMAT(po_mstr.POM_PO_DATE,'%d/%m/%Y') AS 'PO Date',po_mstr.POM_S_COY_NAME AS 'Vendor Name'," _
                        ...
                        & "LEFT OUTER JOIN account_mstr ON account_mstr.AM_ACCT_INDEX=po_details.POD_ACCT_INDEX  " _
                        & "LEFT OUTER JOIN company_dept_mstr b ON account_mstr.AM_DEPT_INDEX = b.CDM_DEPT_INDEX  " _
                        & "WHERE (PO_MSTR.POM_S_COY_ID =  '" & Session("CompanyID") & "') AND POM_PO_DATE >='" & strBeginDate & "' " _
                        & "AND POM_PO_DATE <='" & strEndDate & "' " _
                        & "ORDER BY POM_PO_DATE desc"
                End If
            End With
