...
Public Class ReportFormatN5
    ...
    Private Sub ExportToExcel()
        ...
        Try
            ...
            With cmd
                ...
                If ViewState("ReportType") = "Buyer" Then
                    .CommandText = "SELECT POD_PO_NO AS 'PO Number',DATE_FORMAT(POM_PO_DATE,'%d/%m/%Y') AS 'PO Date', " _
                            ...
                            & "(POD_ORDERED_QTY - POD_DELIVERED_QTY - POD_CANCELLED_QTY) AS 'O/S Qty',DATEDIFF(DATE_ADD(POM_CREATED_DATE, INTERVAL POD_ETD DAY),CURRENT_DATE) AS 'Day(s) to EDD' " _
                            & "FROM PO_MSTR PM " _
                            ...
                            & "WHERE POM_B_COY_ID= '" & Session("CompanyID") & "' AND PM.POM_PO_STATUS IN (1,2,3)  " _
                            & "AND (POD_ORDERED_QTY - POD_DELIVERED_QTY - POD_CANCELLED_QTY)>0 " _
                            & "AND CURRENT_DATE>POM_PO_DATE " _
                            & "AND CURRENT_DATE<=DATE_ADD(PM.POM_CREATED_DATE, INTERVAL POD_ETD DAY) " _
                            & "GROUP BY POM_PO_NO,POD_PO_LINE " _
                            & "ORDER BY DATEDIFF(DATE_ADD(POM_CREATED_DATE, INTERVAL POD_ETD DAY),CURRENT_DATE),POD_PO_NO"
                Else
                    .CommandText = "SELECT POD_PO_NO AS 'PO Number',DATE_FORMAT(POM_PO_DATE,'%d/%m/%Y') AS 'PO Date', " _
                            & "IF(POD_ETD>0,DATE_FORMAT(DATE_ADD(POM_CREATED_DATE, INTERVAL POD_ETD DAY),'%d/%m/%Y'),'Ex-Stock') AS 'EDD'," _
                            & "POM_S_COY_NAME AS 'Vendor Name', IF(PO_DETAILS.POD_VENDOR_ITEM_CODE='&nbsp;','',PO_DETAILS.POD_VENDOR_ITEM_CODE) AS 'Item Code', PO_DETAILS.POD_PRODUCT_DESC AS 'Item Description'," _
                            ...
                            & "LEFT JOIN GRN_MSTR ON GRN_MSTR.GM_DO_Index = DO_Mstr.DOM_DO_Index AND GRN_MSTR.GM_PO_Index = PM.POM_PO_Index " _
                            & "LEFT JOIN GRN_Details ON GRN_Details.GD_GRN_NO = GRN_Mstr.GM_GRN_NO AND GRN_Mstr.GM_B_COY_ID = GRN_Details.GD_B_COY_ID AND GRN_Details.GD_PO_LINE = PO_Details.POD_Po_Line " _
                            & "WHERE POM_S_COY_ID= '" & Session("CompanyID") & "' AND PM.POM_PO_STATUS IN (1,2,3)  " _
                            & "AND (POD_ORDERED_QTY - POD_DELIVERED_QTY - POD_CANCELLED_QTY)>0 " _
                            & "AND CURRENT_DATE>POM_PO_DATE " _
                            & "AND CURRENT_DATE<=DATE_ADD(PM.POM_CREATED_DATE, INTERVAL POD_ETD DAY) " _
                            & "GROUP BY POM_PO_NO,POD_PO_LINE " _
            ...
            da.Fill(ds)
            strFileName = "OutstandingPOReport" & "(" & Format(Now, "ddMMMyyyy") & ").xls"
    ...
    Private Sub ExportToPDF()
        ...
        Try
            ...
            With cmd
                ...
                .CommandType = CommandType.Text
                If ViewState("ReportType") = "Buyer" Then
                    .CommandText = "SELECT POD_PO_NO,POM_CREATED_DATE,POM_PO_DATE, POD_ETD," _
                                & "POM_S_COY_NAME ,PO_DETAILS.POD_VENDOR_ITEM_CODE, PO_DETAILS.POD_PRODUCT_DESC," _
                                & "POD_ORDERED_QTY AS 'POQty',SUM(DOD_DO_QTY) AS 'DOQty',SUM(GD_RECEIVED_QTY) AS 'GRNQty', " _
                                & "POD_REJECTED_QTY, POD_ORDERED_QTY, POD_DELIVERED_QTY, POD_CANCELLED_QTY, company_mstr.CM_COY_NAME, POM_BUYER_ID " _
                                & "FROM PO_MSTR PM " _
                                & "INNER JOIN company_mstr ON pm.POM_B_COY_ID = company_mstr.CM_COY_ID " _
                                & "INNER JOIN PO_DETAILS ON POD_COY_ID = POM_B_COY_ID AND POD_PO_NO = POM_PO_NO " _
                                & "LEFT JOIN do_mstr ON DOM_PO_INDEX=POM_PO_INDEX " _
                                & "LEFT JOIN do_details ON do_mstr.DOM_DO_NO=do_details.DOD_DO_NO AND DOD_PO_LINE=POD_PO_LINE " _
                                & "LEFT JOIN GRN_MSTR ON GRN_MSTR.GM_DO_Index = DO_Mstr.DOM_DO_Index AND GRN_MSTR.GM_PO_Index = PM.POM_PO_Index " _
                                & "LEFT JOIN GRN_Details ON GRN_Details.GD_GRN_NO = GRN_Mstr.GM_GRN_NO AND GRN_Mstr.GM_B_COY_ID = GRN_Details.GD_B_COY_ID AND GRN_Details.GD_PO_LINE = PO_Details.POD_Po_Line " _
                                & "WHERE (POM_B_COY_ID = @prmCoyID) AND PM.POM_PO_STATUS IN (1,2,3)  " _
                                & "AND (POD_ORDERED_QTY - POD_DELIVERED_QTY - POD_CANCELLED_QTY)>0 " _
                                & "AND CURRENT_DATE>POM_PO_DATE " _
                                & "AND CURRENT_DATE<=DATE_ADD(PM.POM_CREATED_DATE, INTERVAL POD_ETD DAY) " _
                                & "GROUP BY POM_PO_NO,POD_PO_LINE " _
                    ...
                    '            & "WHERE     (PM.POM_B_COY_ID = 'tanac') AND (PM.POM_PO_STATUS IN (1, 2, 3)) AND  " _
                    '            & "(po_details.POD_ORDERED_QTY - po_details.POD_DELIVERED_QTY - po_details.POD_CANCELLED_QTY > 0) AND  " _
                    '            & "(CURRENT_DATE > PM.POM_PO_DATE) " _
                    '            & "GROUP BY PM.POM_PO_NO, po_details.POD_PO_LINE"
                Else
                    .CommandText = "SELECT POD_PO_NO,POM_CREATED_DATE,POM_PO_DATE, POD_ETD," _
                                & "POM_S_COY_NAME ,PO_DETAILS.POD_VENDOR_ITEM_CODE, PO_DETAILS.POD_PRODUCT_DESC," _
                                & "POD_ORDERED_QTY AS 'POQty',SUM(DOD_DO_QTY) AS 'DOQty',SUM(GD_RECEIVED_QTY) AS 'GRNQty', " _
                                ...
                                & "LEFT JOIN GRN_MSTR ON GRN_MSTR.GM_DO_Index = DO_Mstr.DOM_DO_Index AND GRN_MSTR.GM_PO_Index = PM.POM_PO_Index " _
                                & "LEFT JOIN GRN_Details ON GRN_Details.GD_GRN_NO = GRN_Mstr.GM_GRN_NO AND GRN_Mstr.GM_B_COY_ID = GRN_Details.GD_B_COY_ID AND GRN_Details.GD_PO_LINE = PO_Details.POD_Po_Line " _
                                & "WHERE (POM_S_COY_ID = @prmCoyID) AND PM.POM_PO_STATUS IN (1,2,3)  " _
                                & "AND (POD_ORDERED_QTY - POD_DELIVERED_QTY - POD_CANCELLED_QTY)>0 " _
                                & "AND CURRENT_DATE>POM_PO_DATE " _
                                & "AND CURRENT_DATE<=DATE_ADD(PM.POM_CREATED_DATE, INTERVAL POD_ETD DAY) " _
                                & "GROUP BY POM_PO_NO,POD_PO_LINE " _
            ...
            da.Fill(ds)
            Dim rptDataSource As New Microsoft.Reporting.WebForms.ReportDataSource("OutstandingPO_DataSetOutstandingPO", ds.Tables(0))
            Dim localreport As New LocalReport
            localreport.DataSources.Clear()
            localreport.DataSources.Add(rptDataSource)
            'localreport.ReportPath = appPath & "Common\Report\POSummary_pdf.rdlc"
            If ViewState("ReportType") = "Buyer" Then
                localreport.ReportPath = dispatcher.direct("Report", "OutstandingPO_pdf.rdlc", "Report") ' Server.MapPath("POSummary_pdf.rdlc")  'dispatcher.direct("Report", "POSummary_pdf.rdlc")
            Else
                localreport.ReportPath = dispatcher.direct("Report", "OutstandingPOVendor_pdf.rdlc", "Report") ' Server.MapPath("POSummary_pdf.rdlc")  'dispatcher.direct("Report", "POSummary_pdf.rdlc")
            End If
            ...
            System.Threading.Thread.Sleep(1000) ' Sleep for 1 seconds
            strFileName = "OutstandingPOReport.pdf"
            'Return PDF
            Me.Response.Clear()
