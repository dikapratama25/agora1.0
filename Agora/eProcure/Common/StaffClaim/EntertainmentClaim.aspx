<%@ Page Language="vb" AutoEventWireup="false" Codebehind="EntertainmentClaim.aspx.vb" Inherits="eProcure.EntertainmentClaim" %>
<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.0 Transitional//EN">
<html>
	<head>
		<title>EntertainmentClaim</title>
        ...
        function clearFields()
		{
		    for (i=0; i < Form1.hidItemLine.value; i++)
			{
			    if (document.getElementById('chkSelection' + i).checked == true){
			        ...
			        ocument.getElementById('txtExRate' + i).disabled = true;
			    }
			}
		}
		...
		function calTotalAmt(id) {
            ...
            var decAmt, decGstAmt, decTotalAmt, decExRate;
            var numbersOnly = /^\d+$/;
        ...
        function calExceedAmt(id) {
            var nopax, entrate, claimAmt;
            var decExdAmt, intPax, intRate, decClaimAmt, decAmt;
            var numbersOnly = /^\d+$/;
            var decimalOnly = /^\s*-?[1-9]\d*(\.\d{1,2})?\s*$/;
            nopax = $('#txtNoPax' + id).val();
            entrate = $('#cboEntRate' + id).val();
            claimAmt = $('#lblTotalAmt' + id).text();
            
            $('#lblExceedAmt' + id).text('');
            if (nopax != '' && entrate != '' && claimAmt != ''){
			    if (numbersOnly.test(nopax)){
			        intPax = parseInt(nopax);
			        intRate = parseInt(entrate.substring(1,3));
			        decAmt = intPax * intRate;
			        decClaimAmt = parseFloat(claimAmt);
			        if (decClaimAmt > decAmt){
			            decExdAmt = decClaimAmt - decAmt;
			            $('#lblExceedAmt' + id).text(decExdAmt.toFixed(2));
			        }
			    }    
            }
        }
	...
	<body class="body" ms_positioning="GridLayout">
		<form id="Form1" method="post" runat="server">
			...
			<% Response.Write(Session("ConstructTableEntertain")) %>
