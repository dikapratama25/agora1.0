...
#End Region
    ...
    Private Sub dtgInvoice_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.DataGridItemEventArgs) Handles dtgInvoice.ItemDataBound
        If (e.Item.ItemType = ListItemType.Item Or e.Item.ItemType = ListItemType.AlternatingItem) Then
            ...
            lnkDocNo.NavigateUrl = dDispatcher.direct("Tracking", "InvoiceDetails.aspx", "pageid=" & strPageId & "&INVNO=" & Common.parseNull(dv("IM_INVOICE_NO")) & "&vcomid=" & Common.parseNull(dv("IM_S_COY_ID")) & "&folder=" & Getfoldertype() & "&status=" & Common.parseNull(dv("IM_INVOICE_STATUS")))
            ...
            strTemp &= "<a href=" & dDispatcher.direct("PO", "PODetail.aspx", "caller=" & "buyer" & "&PO_INDEX=" & dv("IM_PO_INDEX") & "&PO_NO=" & dv("POM_PO_NO") & "&BCoyID=" & Session("CompanyID") & "&side=" & "other" & "&filetype=" & "2" & "&pageid=" & strPageId) & ">" & dv("POM_PO_NO") & "</a><BR>"
            ...
            For intLoop = 0 To intCnt
                ...
                If Not IsDBNull(dvMatch(intLoop)("CDM_DO_NO")) Then
                    ...
                    strURL = dDispatcher.direct("DO", "DODetails.aspx", "caller=" & "buyer" & "&pageid=" & strPageId & "&DONo=" & dvMatch(intLoop)("CDM_DO_NO") & "&POIdx=" & intPoIndex & "&SCoyID=" & dvMatch(intLoop)("CDM_S_COY_ID"))
                    strTemp &= "<a href=""#"" onclick=""PopWindow('" & strURL & "')"">" & dvMatch(intLoop)("CDM_DO_NO") & "</a><BR>"

                    If Not IsDBNull(dvMatch(intLoop)("CDM_GRN_NO")) Then
                        strTemp &= "<a href=" & dDispatcher.direct("GRN", "GRNDetails.aspx", "GRNNo=" & dvMatch(intLoop)("CDM_GRN_NO") & "&BCoyID=" & Session("CompanyID") & "&type=" & "other" & "&pageid=" & strPageId) & ">" & dvMatch(intLoop)("CDM_GRN_NO") & "</a><BR>"
                    End If
                End If
            Next
