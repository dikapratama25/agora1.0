...
#End Region
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If Not IsPostBack Then
            ...
            objGlobal.FillCodeTable(ddlCur, CodeTable.Currency)
            ViewState("role") = getUserRole(blnFO, blnFM)
            'Michelle (8/10/2011) - Issue 986 
            blnFM = objDash.ChkFMCanApprove()
            ViewState("blnFM") = blnFM
            GenerateTab()
            Select Case ViewState("role") 'For this screen, only FM will come here and only the 'Save' and 'Mark As Paid' buttons are available
                Case 2 ' is FO only
                    addListItem("New", "1")
                    addListItem("Sent", "2")
        ...
        'cmdSave.Attributes.Add("onClick", "return resetSummary(0,1);") Commented by Craven 11-05-2011, Reason=WheelScript Unknown error, getting answer from michelle
        cmdSave.Attributes.Add("onClick", "return hidSave();")
        ...
        If Session("CompanyId").ToString.ToUpper = "PAMB" Then
            ...
            lblAction.Text = "Fill in the search criteria and click Search button to list the relevant verified Invoice."
        End If
        'End
    ...
    Function bindgridINV() As String
        ...
        ds = objTrac.getInvoiceTracking_NoDocMatch(Common.Parse(txtDocNo.Text.Replace("\", "\\\\")), txtVendorName.Text, "", "", ViewState("status"), ViewState("IPPstatus"), ViewState("invfrom"), ddldocType.SelectedItem.Value, , , , ddlCur.SelectedItem.Value)
    ...
    Private Sub dtgInvoice_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.DataGridItemEventArgs) Handles dtgInvoice.ItemDataBound
        If (e.Item.ItemType = ListItemType.Item Or e.Item.ItemType = ListItemType.AlternatingItem) Then
            ...
            If IsDBNull(dv("IM_INVOICE_TYPE")) Then
                lnkDocNo.NavigateUrl = dDispatcher.direct("Tracking", "InvoiceDetails.aspx", "Frm=InvoiceVerified" & "&pageid=" & strPageId & "&POINDEX=" & Common.parseNull(dv("IM_PO_INDEX")) & "&INVNO=" & Common.parseNull(dv("IM_INVOICE_NO")) & "&vcomid=" & Common.parseNull(dv("IM_S_COY_ID")) & "&folder=" & Getfoldertype() & "&action=payment&status=" & Common.parseNull(dv("IM_INVOICE_STATUS")))
            End If
            ...
            '' ''strTemp &= "<a href=" & dDispatcher.direct("PO", "PODetail.aspx", "caller=buyer" & "&Frm=InvoiceVerified" & "&PO_INDEX=" & dv("IM_PO_INDEX") & "&PO_NO=" & dv("POM_PO_NO") & "&BCoyID=" & Session("CompanyID") & "&side=other" & "&filetype=2" & "&pageid=" & strPageId) & ">" & dv("POM_PO_NO") & "</a><BR>"
            ' '' ''strTemp &= "<a href=" & dDispatcher.direct("GRN", "GRNDetails.aspx", "GRNNo=" & dv("CDM_GRN_NO") & "&BCoyID=" & Session("CompanyID") & "&type=other" & "&pageid=" & strPageId) & ">" & dv("CDM_GRN_NO") & "</a><BR>"
            ...
            If dv("DOC_TYPE") = "EPROC" Then
                ...
                End Select
            End If
            'Default for document type set to invoice
            ' e.Item.Cells(EnumInv.icInvType).Text = "Invoice"

            'Michelle (12/3/2013) - Issue 1870
            If e.Item.Cells(EnumInv.icInvType).Text = "&nbsp;" Then e.Item.Cells(EnumInv.icInvType).Text = "Invoice"
            ...
            If dv("DOC_TYPE") = "IPP" Then
                ...
                Dim lblPay As Label
                lblPay = e.Item.FindControl("lblPay")
                'hide the ddl of payment method
                cboPay.Visible = False
                lblPay.Visible = True
                txtRemark.Visible = True
                ...
                chk = e.Item.Cells(0).FindControl("chkSelection")
                chk.Visible = False
                lblPay.Text = dv("POM_PAYMENT_METHOD")
                'e.Item.Cells(EnumInv.icPayDate).Text = ""
                If Not IsDBNull(dv("IM_INVOICE_TYPE")) Then
                    lnkDocNo.NavigateUrl = dDispatcher.direct("IPP", "IPPApprovalDetail.aspx", "mode=read&pagefrm=approved&DocumentNo=" & dv("IM_INVOICE_NO") & "&index=" & dv("IM_INVOICE_INDEX"))
                ...
                'Zulham 07122018
                Select Case dv("IM_PAYMENT_TERM").ToString.Trim
                    Case "IBG"
                        e.Item.Cells(EnumInv.icPayTerm).Text = "LOCAL BANK TRANSFER-(RM)"
                    Case "TT"
                        e.Item.Cells(EnumInv.icPayTerm).Text = "TELEGRAPHIC TRANSFER-(FOREIGN CURRENCY)"
                    Case "BC"
                        e.Item.Cells(EnumInv.icPayTerm).Text = "CHEQUE-(RM)"
                    Case "BD"
                        e.Item.Cells(EnumInv.icPayTerm).Text = "BANK DRAFT-(FOREIGN CURRENCY)"
                    Case "CO"
                        e.Item.Cells(EnumInv.icPayTerm).Text = "CASHIER'S ORDER-(RM)"
                End Select
    ...
    Private Function ButtonDislayChanged()
        Select Case cboFolder.SelectedItem.Value
            ...
            Case "2" ' Sent
                ...
                'lblRemark.Text = "This list provides a summary of the Invoice/Payment activity between your company and your vendors."
            Case "3" ' Approval
                FolderChanged(True, False, False, True, False, True, True)
                'ViewState("status") = "2"
                'lblRemark.Text = "This list provides a summary of the Invoice/Payment activity between your company and your vendors."
            Case "4" ' Payment
    ...
    Private Function bindInv(ByVal strType As String, ByVal blnSend As Boolean, ByVal blnFMMassAppr As Boolean)
        ...
        For Each dtr In dtItem.Rows
            If strType = "2" Or strType = "3" Then
                ...
                If strInvAppr2 = "N" Then
                    objTrac.ApproveInvoice(Common.Parse(dtr("InvNo")), Common.Parse(dtr("Vendor")), IIf(strType = "2", "FO", "FM"), dtr("FinRemark"), True, blnHighestLevel)
                Else
                    objTrac.ApproveInvoice(Common.Parse(dtr("InvNo")), Common.Parse(dtr("Vendor")), IIf(strType = "2", "FO", "FM"), dtr("FinRemark"), False, blnHighestLevel)
                End If
            ...
            If strType = "1" Then 'save
                ...
                objTrac.updateAppRemark(dtr("InvIndex"), dtr("FinRemark"), , dtr("PayTerm"))
            End If
            '----Testing 
            ...
            If strType = "4" Then
                ...
                'Michelle (16/10/2012) - 1751
                'dtrInv("AppDate") = Date.Today.ToString
                dtrInv("InvStatus") = invStatus.Paid
                blnSend = False
        ...
        Me.Session.Add("invMarkList", strInv)
        If strInvFail <> "" Then
            Me.Session.Add("invMarkListFail", strInvFail)
        End If
    ...
    Private Sub cmdApprove_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdApprove.Click
        ...
        If Len(Session("invMarkList")) > 0 Then
            strTmp = Mid(Session("invMarkList"), 1, Len(Session("invMarkList")) - 1)
        End If
        Common.NetMsgbox(Me, "Invoice Number(s) " & strTmp & " has/have been marked as paid.", MsgBoxStyle.Information)
        'Common.NetMsgbox(Me, "Invoice Number(s) " & strTmp & " has/have been marked as paid.", "InvoiceVerified.aspx?pageid=" & strPageId)
        bindgridINV()
    End Sub
    ...
    Private Sub GenerateTab()
        ...
        'Session("w_InvVer_tabs") = "<div class=""t_entity""><ul>" & _
        ...
        If ViewState("role") = "3" Then  'FM
            'Zulham 07122018
            Session("w_InvVer_tabs") = "<div class=""t_entity""><ul>" &
        "<li><div class=""space""></div></li>" &
                            "<li><a class=""t_entity_btn"" href=""" & dDispatcher.direct("Tracking", "InvoiceTrackingList.aspx", "pageid=" & strPageId & "&folder=" & "N" & "&status=" & "1") & """><span>Verified Invoice</span></a></li>" &
                            "<li><div class=""space""></div></li>" &
