...
Public Class ConCatBatchUploadDownloadFTN
...
#End Region
    ...
    Private Sub cmdDownload_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdDownload.Click
        ...
        Dim dsGSTRate, dsGSTTaxCode As New DataSet
        Dim objPro As New Products
        ...
        dsGSTTaxCode = objPro.GetItemInfoToExcel("TC")
    ...
    Private Sub cmdDownloadTemplate_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmdDownloadTemplate.Click
        ...
        dsTaxCode = objPro.GetItemInfoToExcel("TC")
    ...
    Private Function CheckContHeader(ByVal dtHeader As DataTable) As Boolean
        ...
        If objExConCatHeader.Action = "New" Then
            ...
            ViewState("GSTRegNo") = objGst.chkGST(ViewState("VCoyId"), "", False)
            ...
            If ViewState("GSTCOD") = True Then
                ...
                If ViewState("GSTRegNo") <> "" And objExConCat.GSTRate <> "" Then
                    strSQL2 = "SELECT CODE_ABBR FROM (SELECT CODE_ABBR, IF((TAX_PERC = '' OR TAX_PERC IS NULL), CODE_DESC, CONCAT(CODE_DESC, ' (', TAX_PERC, '%)')) AS GST_RATE FROM CODE_MSTR INNER JOIN TAX ON TAX_CODE = CODE_ABBR " &
                            "WHERE CODE_CATEGORY = 'GST') tb WHERE GST_RATE = '" & Common.Parse(objExConCat.GSTRate) & "'"
                    GSTRate_Val = objDb2.GetVal(strSQL2)
                    If GSTRate_Val = "" Then
                        drItem.Item("Message") &= "<LI type=square>GST Rate not found.<BR>"
                        bTrue = False
                    Else
                        ...
                        If GSTRate_Val = "EX" Then
                            ...
                            If objDb2.Exist(strSQL2) > 0 Then
                                If objExConCat.TaxCode = "" Then
                                    drItem.Item("Message") &= "<LI type=square>GST Tax Code is required.<BR>"
                                    bTrue = False
                                Else
                                    ...
                                    If GSTTaxCode_Val = "" Then
                                        drItem.Item("Message") &= "<LI type=square>Invalid GST Tax Code found for selected GST Rate.<BR>"
                                        bTrue = False
                                    Else
                ...
                ElseIf ViewState("GSTRegNo") <> "" And objExConCat.GSTRate = "" Then
                    drItem.Item("Message") &= "<LI type=square>GST Rate is required for GST registered company.<BR>"
                    bTrue = False
                ElseIf ViewState("GSTRegNo") = "" And (objExConCat.GSTRate <> "" Or objExConCat.TaxCode <> "") Then
                    drItem.Item("Message") &= "<LI type=square>GST Rate & GST Tax Code is not allowed for non-GST registered company.<BR>"
                    bTrue = False
                Else
                    ViewState("GSTRate") = "N/A"
                    ViewState("GSTTaxCode") = "NR"
                End If
            End If
