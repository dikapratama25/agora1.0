...
#End Region
    ...
    Private Sub dgviewitem_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.DataGridItemEventArgs) Handles dgviewitem.ItemDataBound
        If (e.Item.ItemType = ListItemType.Item Or e.Item.ItemType = ListItemType.AlternatingItem) Then
            ...
            val_Qty.ValidationExpression = "(?!^0*$)(?!^0*\.0*$)^\d{1,6}(\.\d{1,2})?$" '"^([1-9]\d{0,5}\.\d{0,2}|[1-9]\d{0,5}|[0]\d{0,5}\.[1-9]\d{0,1})$" '"(?!^0*$)^\d{1,5}?$"
            val_Qty.ErrorMessage = CStr(intRow + 1) & ". " & ViewState("ValQtyMsg") ' Invalid quantity"
            val_Qty.Text = "?"
...
checkMsg:
    ...
    Private Sub cmd_delete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmd_delete.Click
        ...
        For Each dgItem In dgviewitem.Items
            ...
            If chkItem.Checked Then
            Else
                strItem.Add(New String() {txt_qty.Text, txt_delivery.Text, ""})
            End If
        Next
