...
Public Class InvoiceTrackingListFTN
...
#End Region
    ...
    Private Sub dtgInvoice_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.DataGridItemEventArgs) Handles dtgInvoice.ItemDataBound
        If (e.Item.ItemType = ListItemType.Item Or e.Item.ItemType = ListItemType.AlternatingItem) Then
            ...
            lnkDocNo.NavigateUrl = dDispatcher.direct("Tracking", "InvoiceDetails.aspx", "Frm=" & "InvoiceTrackingList" & "&pageid=" & strPageId & "&INVNO=" & Common.parseNull(dv("IM_INVOICE_NO")) & "&vcomid=" & Common.parseNull(dv("IM_S_COY_ID")) & "&folder=" & Getfoldertype() & "&status=" & Common.parseNull(dv("IM_INVOICE_STATUS")))
            ...
            For Each drRow In dtPR.Rows
                ...
                strURL = "<a href=" & dDispatcher.direct("PR", "PRDetail.aspx", "caller=" & "InvTracking" & "&PageID=" & strPageId & _
                    "&index=" & drRow("PRM_PR_INDEX") & "&PRNo=" & drRow("PRM_PR_NO")) & ">" & drRow("PRM_PR_NO") & "</a>"
                strTemp &= strURL & "<BR>"
            Next

            strTemp &= "<a href=" & dDispatcher.direct("PO", "PODetail.aspx", "Frm=" & "InvoiceTrackingList" & "&caller=" & "buyer" & "&PO_INDEX=" & dv("IM_PO_INDEX") & "&PO_NO=" & dv("POM_PO_NO") & "&BCoyID=" & Session("CompanyID") & "&side=" & "other" & "&filetype=" & "2" & "&pageid=" & strPageId) & ">" & dv("POM_PO_NO") & "</a><BR>"
            'strURL = "../DO/DODetails.aspx?caller=buyer&pageid=" & strPageId & "&DONo=" & dv("CDM_DO_NO") & "&POIdx=" & intPoIndex & "&SCoyID=" & dv("CDM_S_COY_ID")
            'strURL = dDispatcher.direct("Report", "PreviewDO.aspx", "pageid=" & strPageId & "&DONo=" & dv("CDM_DO_NO") & "&POIdx=" & intPoIndex & "&SCoyID=" & dv("CDM_S_COY_ID"))
            ...
            'strTemp &= "<a href=../GRN/GRNDetails.aspx?Frm=InvoiceTrackingList&GRNNo=" & dv("CDM_GRN_NO") & "&BCoyID=" & Session("CompanyID") & "&type=other&pageid=" & strPageId & ">" & dv("CDM_GRN_NO") & "</a><BR>"
        ...
        For Each dtr In dtItem.Rows
            If strType = "2" Or strType = "3" Or strType = "4" Then
                ...
                If strInvAppr2 = "N" Then
                    strMsg = objTrac.ApproveInvoice(Common.Parse(dtr("InvNo")), Common.Parse(dtr("Vendor")), IIf(strType = "2", "FO", "FM"), dtr("FinRemark"), True, blnHighestLevel, False, True)
                Else
                    strMsg = objTrac.ApproveInvoice(Common.Parse(dtr("InvNo")), Common.Parse(dtr("Vendor")), IIf(strType = "2", "FO", "FM"), dtr("FinRemark"), False, blnHighestLevel, False, True)
                End If
    ...
    Private Sub cmdSubmit_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdSubmit.Click
        ...
        If chkOnlyContract(intMsg) = True Then
            ...
            End If
            cmdSubmit.Visible = False
            Common.NetMsgbox(Me, "Invoice Number(s) " & strTmp & " has been submitted for approval.", dDispatcher.direct("Tracking", "InvoiceTrackingList.aspx", "Frm=" & "InvoiceTrackingList" & "&pageid=" & strPageId))
        Else
            If intMsg = 1 Then
    ...
    Private Sub GenerateTab()
        ...
        '                    "<li><a class=""t_entity_btn_selected"" href=""#""><span>New Invoice</span></a></li>" & _
        '                         "<li><div class=""space""></div></li>" & _
        '                    "<li><a class=""t_entity_btn"" href=""InvoiceVerifiedTrackingList.aspx?folder=S" & "&status=1&pageid=" & strPageId & """><span>Verified Invoice</span></a></li>" & _
        '                         "<li><div class=""space""></div></li>" & _
