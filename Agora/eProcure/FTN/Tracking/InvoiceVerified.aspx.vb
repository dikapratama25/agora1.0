...
#End Region
    ...
    Private Sub dtgInvoice_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.DataGridItemEventArgs) Handles dtgInvoice.ItemDataBound
        If (e.Item.ItemType = ListItemType.Item Or e.Item.ItemType = ListItemType.AlternatingItem) Then
            ...
            lnkDocNo.NavigateUrl = dDispatcher.direct("Tracking", "InvoiceDetails.aspx", "Frm=InvoiceVerified" & "&pageid=" & strPageId & "&POINDEX=" & Common.parseNull(dv("POM_PO_INDEX")) & "&INVNO=" & Common.parseNull(dv("IM_INVOICE_NO")) & "&vcomid=" & Common.parseNull(dv("IM_S_COY_ID")) & "&folder=" & Getfoldertype() & "&status=" & Common.parseNull(dv("IM_INVOICE_STATUS")))
            ...
            'strURL = dDispatcher.direct("Report", "PreviewDO.aspx", "caller=buyer" & "&pageid=" & strPageId & "&DONo=" & dv("CDM_DO_NO") & "&POIdx=" & intPoIndex & "&SCoyID=" & dv("CDM_S_COY_ID"))
        ...
        For Each dtr In dtItem.Rows
            If strType = "2" Or strType = "3" Or strType = "4" Then
                ...
                If strInvAppr2 = "N" Then
                    objTrac.ApproveInvoice(Common.Parse(dtr("InvNo")), Common.Parse(dtr("Vendor")), IIf(strType = "2", "FO", "FM"), dtr("FinRemark"), True, blnHighestLevel, False)
                Else
                    objTrac.ApproveInvoice(Common.Parse(dtr("InvNo")), Common.Parse(dtr("Vendor")), IIf(strType = "2", "FO", "FM"), dtr("FinRemark"), False, blnHighestLevel, False)
                End If
