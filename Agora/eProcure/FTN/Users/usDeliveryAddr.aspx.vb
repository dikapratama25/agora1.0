...
Imports System.text
Public Class usDeliveryAddrFTN
...
#End Region
    ...
    Protected Overrides Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        'btnRemoveSelected.Attributes.Add("onclick", "return confirm('Warning! This will remove ALL selected addresses and NO Delivery Address will be assigned to this Buyer. Are you Sure?');")
    End Sub
        ...
        Select Case viewstate("Role")
            Case "SK"
                If objUsers.IsRoleSelected(objUserDetails.UserGroup, FixedRole.Store_Keeper) And objUsers.IsRoleSelected(objUserDetails.UserGroup, FixedRole.Second_Level_Receiver) Then
                    ds = objUsers.SearchUserAddress(viewstate("UserId"), txt_Code.Text, txt_City.Text, cbo_State.SelectedValue)
                ElseIf objUsers.IsRoleSelected(objUserDetails.UserGroup, FixedRole.Store_Keeper) And Not objUsers.IsRoleSelected(objUserDetails.UserGroup, FixedRole.Second_Level_Receiver) Then
                    ds = objUsers.SearchUserAddress(viewstate("UserId"), txt_Code.Text, txt_City.Text, cbo_State.SelectedValue, 1)
                ElseIf Not objUsers.IsRoleSelected(objUserDetails.UserGroup, FixedRole.Store_Keeper) And objUsers.IsRoleSelected(objUserDetails.UserGroup, FixedRole.Second_Level_Receiver) Then
                    ds = objUsers.SearchUserAddress(viewstate("UserId"), txt_Code.Text, txt_City.Text, cbo_State.SelectedValue, 2)
                Else
                    ds = objUsers.SearchUserAddress(viewstate("UserId"), txt_Code.Text, txt_City.Text, cbo_State.SelectedValue)
                End If

            Case "B"
                ds = objUsers.SearchUserAddress(viewstate("UserId"), txt_Code.Text, txt_City.Text, cbo_State.SelectedValue, IIf(viewstate("AddrType") = "D", 0, 1), True, FixedRole.Buyer)
        End Select
