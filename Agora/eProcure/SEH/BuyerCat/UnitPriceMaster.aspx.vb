...
Public Class UnitPriceMasterSEH
    ...
    Protected WithEvents hidvline As System.Web.UI.WebControls.Label
    Protected WithEvents hidVendor As System.Web.UI.WebControls.Label
...
#End Region
    ...
    Protected Overrides Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If Not Page.IsPostBack Then
            If Common.parseNull(Request.QueryString("vid")) = "" Then
                lblVendor.Text = ""
                hidVendor.Text = ""
                hidvline.Text = ""
                Common.NetMsgbox(Me, "Please type vendor company")
                'RebuildClose()
                cmd_Save.Enabled = False
                cmd_Add.Enabled = False
            Else
                lblVendor.Text = objDb.GetVal("SELECT CM_COY_NAME FROM company_mstr WHERE CM_COY_ID = '" & Request.QueryString("vid") & "'")
                hidVendor.Text = Request.QueryString("vid")
                hidvline.Text = Request.QueryString("vline")
            End If
    ...
    Private Sub cmd_Save_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmd_Save.Click
        ...
        If intNumeric2 = False Then
            strMsg = "<ul type='disc'><li>" & "Unit Price " & objGLO.GetErrorMessage("00013") & "<ul type='disc'></ul></li></ul>"
        End If
        ...
        If intEmpty2 = False Then
            strMsg = "<ul type='disc'><li>" & "Unit Price cannot be blank or zero" & "<ul type='disc'></ul></li></ul>"
        End If
    ...
    Private Function ConstructTable(Optional ByVal aryVolume As ArrayList = Nothing, Optional ByVal strMode As String = "")
        ...
        table = "<table class=""grid"" style=""margin-top:10px; border-collapse:collapse; line-height:20px; "" >" & _
                "<tr class=""TableHeader""><td style=""width:180px;"">Volume</td><td style=""width:60px;"">Unit Price</td></tr>" & _
                strrow & _
                "</table>"
    ...
    Private Function BuildSave()
        ...
        If Not Session("aryPrice") Is Nothing Then
            ...
            Do While i < aryPrice.Count
                If aryPrice(i)(3) = Request.QueryString("vline") Then
                    aryPrice.RemoveAt(i)
                    i = 0
                Else
                    i = i + 1
            ...
            For i = 0 To arySetPrice.Count - 1
                aryPrice.Add(New String() {arySetPrice(i)(0), arySetPrice(i)(1), Request.QueryString("vid"), Request.QueryString("vline"), "Save", "", ""})
            Next
    ...
    Private Function Rebuild()
        ...
        For i = 0 To arySetPrice.Count - 1
            If arySetPrice(i)(3) = Request.QueryString("vline") Then
                If Request.Form("txtVolume" & i) <> "" Or Request.Form("txtPrice" & i) <> "" Then
                    aryTemp.Add(New String() {Request.Form("txtVolume" & i), Request.Form("txtPrice" & i), Request.QueryString("vid"), Request.QueryString("vline"), "Save", arySetPrice(i)(0), arySetPrice(i)(1)})
                End If
            End If
        Next
        ...
        If intAllTrue = True Then
            For i = 0 To c - 1
                aryTemp2.Add(New String() {"", "", Request.QueryString("vid"), Request.QueryString("vline"), "Add", "", ""})
            Next
    ...
    Private Function BuildLine()
        ...
        For i = 0 To arySetPrice.Count - 1
            ...
            End If
        Next
        arySetPrice.Add(New String() {Request.Form("txtVolume" & found + 1), Request.Form("txtPrice" & found + 1), Request.QueryString("vid"), Request.QueryString("vline"), "Add", "", ""})
    ...
    Private Function BuildRow()
        ...
        If Session("aryPrice") Is Nothing Then 'Empty
            If Request.QueryString("mode") = "mod" Then
                dsPrice = objCat_Ext.GetUnitPrice(Request.QueryString("itemid"), hidvline.Text)
                ...
                If dsPrice.Tables(0).Rows.Count > 0 Then
                    ...
                    For i = 0 To ViewState("Row") - 1
                        If i = 0 Then
                            arySetPrice.Add(New String() {"1.00", Request.Form("txtPrice" & i), Request.QueryString("vid"), Request.QueryString("vline"), "Add", "", ""})
                        Else
                            arySetPrice.Add(New String() {Request.Form("txtVolume" & i), Request.Form("txtPrice" & i), Request.QueryString("vid"), Request.QueryString("vline"), "Add", "", ""})
                        End If

                    Next

                    For i = 0 To dsPrice.Tables(0).Rows.Count - 1
                        arySetPrice(i)(0) = Format(CDbl(dsPrice.Tables(0).Rows(i)("PVP_VOLUME")), "##0.00")
                        arySetPrice(i)(1) = Format(CDbl(dsPrice.Tables(0).Rows(i)("PVP_VOLUME_PRICE")), "##0.00")
        ...
        Else ' Session Not Empty 1.Reload   2.After Add Line
            ...
            For i = 0 To temp_A.Count - 1
                If temp_A(i)(3) = Request.QueryString("vline") Then
                    found = True
                    Exit For
                End If
            ...
            If found = False Then
                If Request.QueryString("mode") = "mod" Then
                    dsPrice = objCat_Ext.GetUnitPrice(Request.QueryString("itemid"), Request.QueryString("vline"))
