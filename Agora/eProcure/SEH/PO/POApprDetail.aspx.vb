...
Public Class POApprDetail_SEH
    ...
    Protected WithEvents lbl_VC As System.Web.UI.WebControls.Label
    Protected WithEvents lbl_DT As System.Web.UI.WebControls.Label

    Protected WithEvents lblVenCode As System.Web.UI.WebControls.Label
    ...
    Protected WithEvents dtgPOListStk As System.Web.UI.WebControls.DataGrid
    ...
    Dim intRemarkCol As Integer = 21 '20 '19
...
#End Region
    ...
    Protected Overrides Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        SetGridProperty(dtgPOListStk)

        If Not Page.IsPostBack Then
            GenerateTab()
            Dim objPO As New PurchaseOrder_AO
            Dim objPO_Ext As New PurchaseOrder_AO_Ext
            Dim objP_Ext As New PurchaseOrder_Ext
            dsAllInfo = objPO_Ext.getPOForAppr(Request.QueryString("PONO"), Request.QueryString("index"))
            ...
            For c = 0 To 3 - 1
                If c = 0 Then
                    dtDate = Now.Date.AddMonths(-3).AddDays(-Now.Date.Day + 1)
                ElseIf c = 1 Then
                    dtDate = Now.Date.AddMonths(-2).AddDays(-Now.Date.Day + 1)
                Else
                    dtDate = Now.Date.AddMonths(-1).AddDays(-Now.Date.Day + 1)
                End If
            ...
            If objP_Ext.chkPOItemLine(Request.QueryString("PONo"), "B", Session("CompanyId")) Then
            ...
            If ViewState("BCM") > 0 Then
                'dsBCM = objBCM.getBCMListByUserFrmPR(Session("UserId"), "")
                'dsBCM = objBCM.getBCMListByUserNew(Session("UserId"))
                dtBCM = objBCM.getBCMListByCompanyNew()
                ' _Yap Use Tables(1) instate of Tables(0)
                ' Tables(1) = List Available : This bez not each use have the budget full list for the Approval user.
                ' Tables(0) = Selected List.
                'dtBCM = dsBCM.Tables(1)
            End If
            ...
            Dim strBCM As String
            Dim blnExceed As Boolean
            blnExceed = objBudget.checkBCMPO(Request.QueryString("PONO"), dtBCMChk, strBCM)

            objBudget = Nothing

            If blnExceed Then
                Common.NetMsgbox(Me, "PO value is more than the Operating Budget Amount.", MsgBoxStyle.Exclamation)
            End If

            'lblTitle.Text = "Purchase Requisition Approval Details"
            If ViewState("ApprType") = "2" Then
                'lblTitle.Text = lblTitle.Text & " (Endorsement)"
                cmdAppPO.Text = "Endorse PO"
            End If
        ...
        Me.dtgPOList.Columns(2).Visible = True
        Me.dtgPOList.Columns(10).Visible = True
        Me.dtgPOListStk.Columns(2).Visible = True
        Me.dtgPOListStk.Columns(10).Visible = True
        Me.dtgAppFlow.Columns(2).Visible = True
        ...
        If Asset.AssetGroupMstr = False Then
            dtgPOList.Columns(EnumPO.icAssetCode).Visible = False
            dtgPOListStk.Columns(EnumPOStk.icAssetCode).Visible = False
        End If
    ...
    Private Sub renderPRHeader()
        ...
        If dtHeader.Rows.Count > 0 Then
            ...
            If IsDBNull(dtHeader.Rows(0)("DEL_NAME")) Then
                lblDelTerm.Text = Common.parseNull(dtHeader.Rows(0)("POM_DEL_CODE"))
            Else
                lblDelTerm.Text = Common.parseNull(dtHeader.Rows(0)("POM_DEL_CODE")) & " (" & Common.parseNull(dtHeader.Rows(0)("DEL_NAME")) & ")"
            End If

            lblVenCode.Text = Common.parseNull(dtHeader.Rows(0)("POM_VENDOR_CODE"))
            ...
            If strRFQIndex = "" Then
                ...
                dtgPOListStk.Columns(EnumPOStk.icQty).HeaderText = "Qty"
            Else
            ...
            If Not IsDBNull(dtHeader.Rows(0)("POM_SUBMIT_DATE")) Then
                ViewState("GstPO") = objGst.chkGSTCOD(Format(dtHeader.Rows(0)("POM_SUBMIT_DATE"), "dd/MM/yyyy"))
            End If
        End If

        If Session("blnTrue") = True Then
            'lbl_DT.Visible = False
            'lblDelTerm.Visible = False
            lbl_VC.Visible = False
            lblVenCode.Visible = False
        Else
            'lbl_DT.Visible = True
            'lblDelTerm.Visible = True
            lbl_VC.Visible = True
    ...
    Private Sub renderPRDetail()
        ...
        If strRFQIndex = "" Then
            ...
            Else
                intSubTotalCol = EnumPOStk.icSubTotal - 1
            End If
        ...
        Else
            ...
            Else
                intSubTotalCol = EnumPOStk.icSubTotal
            End If
        End If
        ...
        If ViewState("GstPO") = True Then
            dtgPOList.Columns(EnumPO.icTax).Visible = False
            dtgPOList.Columns(EnumPO.icGstAmt).Visible = True
            dtgPOList.Columns(EnumPO.icGstRate).Visible = True
            dtgPOListStk.Columns(EnumPOStk.icTax).Visible = False
            dtgPOListStk.Columns(EnumPOStk.icGstAmt).Visible = True
            dtgPOListStk.Columns(EnumPOStk.icGstRate).Visible = True
            'Stage 3 Enhancement (GST-0010) - 20/07/2015 - CH
            dtgPOList.Columns(EnumPO.icGstTaxCode).Visible = True
            dtgPOListStk.Columns(EnumPOStk.icGstTaxCode).Visible = True
        Else
            dtgPOList.Columns(EnumPO.icTax).Visible = True
            dtgPOList.Columns(EnumPO.icGstAmt).Visible = False
            dtgPOList.Columns(EnumPO.icGstRate).Visible = False
            dtgPOListStk.Columns(EnumPOStk.icTax).Visible = True
            dtgPOListStk.Columns(EnumPOStk.icGstAmt).Visible = False
            dtgPOListStk.Columns(EnumPOStk.icGstRate).Visible = False
            'Stage 3 Enhancement (GST-0010) - 20/07/2015 - CH
            dtgPOList.Columns(EnumPO.icGstTaxCode).Visible = False
            dtgPOListStk.Columns(EnumPOStk.icGstTaxCode).Visible = False
        End If
    ...
    Private Sub renderPRAdd()
        ...
        AddRow(intSubTotalCol, "Grand Total ", CDbl(Format(dblGrandTotal, "#0.00")), False)
        'dtgPOList.Columns(EnumPO.icTax).Visible = True
        'dtgPOListStk.Columns(EnumPO.icTax).Visible = True
        If ViewState("GstPO") = True Then
            dtgPOList.Columns(EnumPO.icTax).Visible = False
            dtgPOList.Columns(EnumPO.icGstAmt).Visible = True
            dtgPOList.Columns(EnumPO.icGstRate).Visible = True
    ...
    Sub AddRow(ByVal intCell As Integer, ByVal strLabel As String, ByVal dblTotal As Double, ByVal blnShowGST As Boolean)
        ...
        '' ''row.Cells(0).ColumnSpan = 5 ' intCell
        ...
        Dim intTotalCol As Integer
        If Session("blnTrue") = True Then
            intCell = EnumPO.icSubTotal
            intTotalCol = EnumPO.icRemark + intCnt - 1
        Else
            intCell = EnumPOStk.icSubTotal
            intTotalCol = EnumPOStk.icRemark + intCnt - 1
        End If
        ...
        row.Cells(0).ColumnSpan = intCell - 8
        row.Cells(1).ColumnSpan = intCell - 3 '2
        Dim Asset As New PurchaseOrder_Buyer
        If Asset.AssetGroupMstr = False Then
            row.Cells(1).ColumnSpan = intCell - 4
        End If
        row.Cells(1).Text = strLabel
        row.Cells(1).Font.Bold = True
        row.Cells(1).HorizontalAlign = HorizontalAlign.Right
        ...
        'row.Cells(3).ColumnSpan = intTotalCol - (intCell - 9)
        ...
        If Session("blnTrue") = True Then
            dtgPOList.Controls(0).Controls.Add(row)
        Else
            dtgPOListStk.Controls(0).Controls.Add(row)
        End If
    ...
    Private Function Bindgrid(Optional ByVal pSorted As Boolean = False) As String
        ...
        If intPageRecordCnt > 0 Then
            If Session("blnTrue") = True Then
                ...
                dtgPOList.DataBind()
            Else
                dtgPOListStk.DataSource = dvViewPR
                dtgPOListStk.DataBind()
            End If
        Else
            If Session("blnTrue") = True Then
                dtgPOList.DataBind()
        ...
        If Session("blnTrue") = True Then
            dtgPOList.Visible = True
            dtgPOListStk.Visible = False
        Else
            dtgPOList.Visible = False
            dtgPOListStk.Visible = True
        End If
    ...
    Private Sub dtgPOList_ItemDataBound(ByVal sender As System.Object, ByVal e As System.Web.UI.WebControls.DataGridItemEventArgs) Handles dtgPOList.ItemDataBound
        ...
        If (e.Item.ItemType = ListItemType.Item Or e.Item.ItemType = ListItemType.AlternatingItem) Then
            ...
            e.Item.Cells(EnumPO.icGstRate).Text = Common.parseNull(dv("GST_RATE"))
            e.Item.Cells(EnumPO.icCost).Text = Format(Common.parseNull(dv("POD_UNIT_COST"), 0), "#,##0.0000")
            'Stage 3 Enhancement (GST-0010) - 20/07/2015 - CH
            e.Item.Cells(EnumPO.icGstTaxCode).Text = Common.parseNull(dv("POD_GST_INPUT_TAX_CODE"))
            ...
            If Common.parseNull(dv("POD_GST"), 0) = 0 Then
                ...
                e.Item.Cells(EnumPO.icGstAmt).Text = "0.00"
            Else
                ...
                e.Item.Cells(EnumPO.icGstAmt).Text = Format(dblGstAmt, "#,##0.00")
            End If
            ...
            If blnCustomField Then
                ...
                Try
                    ...
                    If e.Item.Cells(EnumPO.icRemark).Controls(0).GetType Is GetType(Label) Then
                        For i = 0 To intCnt - 1
                            strFieldNo = strC(i)
                            'lblCustom = e.Item.Cells(intRemarkCol + i).Controls(0)
                            lblCustom = e.Item.Cells(EnumPO.icRemark + i).Controls(0)
    ...
    Private Sub dtgPOListStk_ItemCreated(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.DataGridItemEventArgs) Handles dtgPOListStk.ItemCreated
        Grid_ItemCreated(dtgPOListStk, e)
        ...
        If e.Item.ItemType = ListItemType.Header Then
            ...
            For i = 0 To aryTempList.Count - 1
                ...
                If i = 0 Then
                    e.Item.Cells(EnumPOStk.icPast1stMthUsage).Text = strMonth & " Qty Usage"
                ElseIf i = 1 Then
                    e.Item.Cells(EnumPOStk.icPast2ndMthUsage).Text = strMonth & " Qty Usage"
                ElseIf i = 2 Then
                    e.Item.Cells(EnumPOStk.icPast3rdMthUsage).Text = strMonth & " Qty Usage"
                End If
            Next
            ...
            For i = 0 To aryTempList.Count - 1
                strMonth = objpo_ext.get_month(CInt(aryTempList(i)(0)))
                If i = 0 Then
                    e.Item.Cells(EnumPOStk.icNext1stMthQty).Text = strMonth & " Usage"
                ElseIf i = 1 Then
                    e.Item.Cells(EnumPOStk.icNext2ndMthQty).Text = strMonth & " Usage"
                ElseIf i = 2 Then
                    e.Item.Cells(EnumPOStk.icNext3rdMthQty).Text = strMonth & " Usage"
                End If
            Next
        End If
    End Sub

    Private Sub dtgPOListStk_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.DataGridItemEventArgs) Handles dtgPOListStk.ItemDataBound
        Dim i As Integer
        Dim intPRLine As Integer
        Dim strFieldNo, strDAddr As String
        Dim dr() As DataRow
        ...
        If (e.Item.ItemType = ListItemType.Item Or e.Item.ItemType = ListItemType.AlternatingItem) Then
            ...
            e.Item.Cells(EnumPOStk.icGstRate).Text = Common.parseNull(dv("GST_RATE"))
            e.Item.Cells(EnumPOStk.icCost).Text = Format(Common.parseNull(dv("POD_UNIT_COST"), 0), "#,##0.0000")
            'Stage 3 Enhancement (GST-0010) - 20/07/2015 - CH
            e.Item.Cells(EnumPOStk.icGstTaxCode).Text = Common.parseNull(dv("POD_GST_INPUT_TAX_CODE"))
            '2015-06-22: CH: Rounding issue (Prod issue)
            'dblAmt = Common.parseNull(dv("POD_UNIT_COST"), 0) * Common.parseNull(dv("POD_ORDERED_QTY"), 0)
            dblAmt = CDec(Format(Common.parseNull(dv("POD_UNIT_COST"), 0) * Common.parseNull(dv("POD_ORDERED_QTY"), 0), "###0.00"))
            ...
            If Common.parseNull(dv("POD_GST"), 0) = 0 Then
                'e.Item.Cells(EnumPO.icCost).Text = e.Item.Cells(EnumPO.icCost).Text & " (no tax)"
                e.Item.Cells(EnumPOStk.icCost).Text = e.Item.Cells(EnumPOStk.icCost).Text
                dblNoTaxTotal = dblNoTaxTotal + dblAmt
                intNoGSTcnt += 1
                intTotItem += 1
                '//for testing
                e.Item.Cells(EnumPOStk.icTax).Text = "0.00"
                e.Item.Cells(EnumPOStk.icGstAmt).Text = "0.00" '"n.a."
            Else
                dblTaxTotal = dblTaxTotal + dblAmt
                intGSTcnt += 1
                intTotItem += 1
                '//for testing
                dblGstAmt = dblAmt * (Common.parseNull(dv("POD_GST"), 0) / 100)
                e.Item.Cells(EnumPOStk.icTax).Text = Format(dblGstAmt, "#,##0.00")
                e.Item.Cells(EnumPOStk.icGstAmt).Text = Format(dblGstAmt, "#,##0.00")
            End If
            ...
            e.Item.Cells(EnumPOStk.icSubTotal).Text = Format(dblAmt, "#,##0.00")

            e.Item.Cells(EnumPOStk.icDAddr).ToolTip = strDAddr
            e.Item.Cells(EnumPOStk.icDAddr).Text = Common.parseNull(dv("POD_D_ADDR_CODE"))
            If blnCustomField Then
                intPRLine = Convert.ToInt32(e.Item.Cells(EnumPOStk.icSNo).Text)
                Dim lblCustom As Label
                Try
                    Dim typename As String = e.GetType().ToString()
                    'e.Item.Cells
                    ...
                    'If e.Item.Cells(intRemarkCol).Controls(0).GetType Is GetType(Label) Then
                    If e.Item.Cells(EnumPOStk.icRemark).Controls(0).GetType Is GetType(Label) Then
                        For i = 0 To intCnt - 1
                            strFieldNo = strC(i)
                            'lblCustom = e.Item.Cells(intRemarkCol + i).Controls(0)
                            lblCustom = e.Item.Cells(EnumPOStk.icRemark + i).Controls(0)
                            dr = dt.Select("PCD_FIELD_NO=" & strFieldNo & " AND PCD_PR_LINE=" & intPRLine)
                            If dr.Length > 0 Then
                                lblCustom.Text = Common.parseNull(dr(0)("PCD_FIELD_VALUE"))
                            End If
            ...
            If Not IsDBNull(dv("POD_ACCT_INDEX")) Then
                If Not dtBCM Is Nothing Then
                    ...
                    drTemp = dtBCM.Select("Acct_Index=" & dv("POD_ACCT_INDEX"))
                    If drTemp.Length > 0 Then
                        e.Item.Cells(EnumPOStk.icBCM).Text = drTemp(0)("Acct_List")
                    End If
                End If
            End If
            ...
            Dim dblQty, dbl1stMthQty, dbl2ndMthQty, dbl3rdMthQty, dbl3MthAve, dblCurrQty, dblStkOnHand, dblPOBalance, dblPOInProgress, dblFC As Double
            Dim j, intMth, intYear As Integer
            Dim strMonth As String

            'ArrayList = Session("aryPast3Month")
            'For j = 0 To arraylist.Count - 1
            '    intMth = CInt(arraylist(j)(0))  'Month
            '    intYear = CInt(arraylist(j)(1)) 'Year
            '    dblQty = objpo_ext.get_POQtyByMonth(intMth, intYear, Session("side"), Request(Trim("BCoyID")))
            '    strMonth = objpo_ext.get_month(intMth)

            '    If j = 0 Then
            '        e.Item.Cells(EnumPOStk.icPast1stMthUsage).Text = Format(dblQty, "###0.00")
            '        dbl1stMthQty = dblQty
            '    ElseIf j = 1 Then
            '        e.Item.Cells(EnumPOStk.icPast2ndMthUsage).Text = Format(dblQty, "###0.00")
            '        dbl2ndMthQty = dblQty
            '    ElseIf j = 2 Then
            '        e.Item.Cells(EnumPOStk.icPast3rdMthUsage).Text = Format(dblQty, "###0.00")
            '        dbl3rdMthQty = dblQty
            '    End If

            'Next

            'dbl3MthAve = (dbl1stMthQty + dbl1stMthQty + dbl1stMthQty) / 3
            'e.Item.Cells(EnumPOStk.icPast3MthAve).Text = Format(dbl3MthAve, "###0.00")

            If Not IsDBNull(dv("POD_PREV1_QTY")) Then
                e.Item.Cells(EnumPOStk.icPast1stMthUsage).Text = Format(dv("POD_PREV1_QTY"), "###0.00")
            Else
                e.Item.Cells(EnumPOStk.icPast1stMthUsage).Text = Format(0, "###0.00")
            End If

            If Not IsDBNull(dv("POD_PREV2_QTY")) Then
                e.Item.Cells(EnumPOStk.icPast2ndMthUsage).Text = Format(dv("POD_PREV2_QTY"), "###0.00")
            Else
                e.Item.Cells(EnumPOStk.icPast2ndMthUsage).Text = Format(0, "###0.00")
            End If

            If Not IsDBNull(dv("POD_PREV3_QTY")) Then
                e.Item.Cells(EnumPOStk.icPast3rdMthUsage).Text = Format(dv("POD_PREV3_QTY"), "###0.00")
            Else
                e.Item.Cells(EnumPOStk.icPast3rdMthUsage).Text = Format(0, "###0.00")
            End If

            If Not IsDBNull(dv("POD_PREV_AVG")) Then
                e.Item.Cells(EnumPOStk.icPast3MthAve).Text = Format(dv("POD_PREV_AVG"), "###0.00")
            Else
                e.Item.Cells(EnumPOStk.icPast3MthAve).Text = Format(0, "###0.00")
            End If

            If Not IsDBNull(dv("POD_CURR_QTY")) Then
                dblCurrQty = dv("POD_CURR_QTY")
                e.Item.Cells(EnumPOStk.icCurrMthQty).Text = Format(dv("POD_CURR_QTY"), "###0.00")
            Else
                dblCurrQty = 0
                e.Item.Cells(EnumPOStk.icCurrMthQty).Text = Format(0, "###0.00")
            End If

            If Not IsDBNull(dv("POD_NEXT1_QTY")) Then
                e.Item.Cells(EnumPOStk.icNext1stMthQty).Text = Format(dv("POD_NEXT1_QTY"), "###0.00")
            Else
                e.Item.Cells(EnumPOStk.icNext1stMthQty).Text = Format(0, "###0.00")
            End If

            If Not IsDBNull(dv("POD_NEXT2_QTY")) Then
                e.Item.Cells(EnumPOStk.icNext2ndMthQty).Text = Format(dv("POD_NEXT2_QTY"), "###0.00")
            Else
                e.Item.Cells(EnumPOStk.icNext2ndMthQty).Text = Format(0, "###0.00")
            End If

            If Not IsDBNull(dv("POD_NEXT3_QTY")) Then
                e.Item.Cells(EnumPOStk.icNext3rdMthQty).Text = Format(dv("POD_NEXT3_QTY"), "###0.00")
            Else
                e.Item.Cells(EnumPOStk.icNext3rdMthQty).Text = Format(0, "###0.00")
            End If

            If Not IsDBNull(dv("POD_STOCK_ON_HAND_QTY")) Then
                dblStkOnHand = CDbl(dv("POD_STOCK_ON_HAND_QTY"))
                e.Item.Cells(EnumPOStk.icStkOnHand).Text = Format(dv("POD_STOCK_ON_HAND_QTY"), "###0.00")
            Else
                dblStkOnHand = 0
                e.Item.Cells(EnumPOStk.icStkOnHand).Text = Format(0, "###0.00")
            End If

            If Not IsDBNull(dv("POD_PO_BALANCE_QTY")) Then
                dblPOBalance = CDbl(dv("POD_PO_BALANCE_QTY"))
                e.Item.Cells(EnumPOStk.icPOBalance).Text = Format(dv("POD_PO_BALANCE_QTY"), "###0.00")
            Else
                dblPOBalance = 0
                e.Item.Cells(EnumPOStk.icPOBalance).Text = Format(0, "###0.00")
            End If

            If Not IsDBNull(dv("POD_PO_IN_PROGRESS_QTY")) Then
                dblPOInProgress = CDbl(dv("POD_PO_IN_PROGRESS_QTY"))
                e.Item.Cells(EnumPOStk.icPOInProgress).Text = Format(dv("POD_PO_IN_PROGRESS_QTY"), "###0.00")
            Else
                dblPOInProgress = 0
                e.Item.Cells(EnumPOStk.icPOInProgress).Text = Format(0, "###0.00")
            End If

            If dblCurrQty = 0 Then
                e.Item.Cells(EnumPOStk.icForecast).Text = Format(0, "###0.00")
            Else
                dblFC = (dblStkOnHand + dblPOBalance + dblPOInProgress) / dblCurrQty
                e.Item.Cells(EnumPOStk.icForecast).Text = Format(dblFC, "###0.00")
            End If

            If Not IsDBNull(dv("POD_LEAD_TIME")) Then
                e.Item.Cells(EnumPOStk.icLeadTime).Text = dv("POD_LEAD_TIME")
            Else
                e.Item.Cells(EnumPOStk.icLeadTime).Text = 0
            End If
    ...
    Private Sub addDataGridColumn()
        ...
        For i = 0 To intCnt - 1
            ...
            dtgPOListStk.Columns.AddAt(dtgPOListStk.Columns.Count - 1, col)
    ...
    Private Sub cmdHoldPO_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdHoldPO.Click
        ...
        Else
            dsPODetail = bindPODetail()
            strMsg = objPO.HoldPO(Session("lblPO"), ViewState("POIndex"), ViewState("CurrentAppSeq"), strRemark, Request.QueryString("AO"), Request.QueryString("relief"), dsPODetail)
            objPO = Nothing
            strUrl = lnkBack.NavigateUrl
            Common.NetMsgbox(Me, strMsg, strUrl, MsgBoxStyle.Exclamation)
            'cmdRejectPO.Visible = False
    ...
    '' ''        Common.NetMsgbox(Me, "Please enter Remarks.", MsgBoxStyle.Information)
    '' ''    Else
    '' ''        strMsg = objPR.HoldPR(Session("lblPO"), ViewState("PRIndex"), ViewState("CurrentAppSeq"), strRemark, Request.QueryString("AO"), Request.QueryString("relief"))
    '' ''        objPR = Nothing
