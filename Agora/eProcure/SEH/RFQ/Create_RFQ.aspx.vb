...
Public Class Create_RFQ_SEH
    ...
    Dim objread As New RFQ_User_Ext()
...
#End Region
    ...
    Protected Overrides Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        imageid = 0
        Dim a As New AppGlobals
        Dim a_Ext As New AppGlobals_Ext
        blnPaging = False
        blnSorting = False
        ...
        If Not Page.IsPostBack Then
            ...
            If Request(Trim("caller")) = "VendorSearch" Or Request(Trim("caller")) = "PurchaserSearch" Then 'ie raise from Vendor/Purchaser Item Search
                ...
                For i = 0 To aryProdCode.Count - 1
                    ...
                    End If
                Next
                objrfq_Ext.add_RFQCatSearch_TEMP(strProdList)
            End If
        ...
        If (Session("back") = "1") Then
            ...
            If edit = "1" Then
                ...
                objrfq_Ext.read_rfqMstr(objread, "", rfq_id)
        ...
        If Not Page.IsPostBack Then
            ...
            If Session("rfq_id") <> "" Then
                'checkdata As String = 
                objrfq_Ext.read_rfqMstr(objread, Session("RFQ_name"), Session("rfq_id"), Session("rfq_num"))
                Common.SelDdl(objread.cur_code, ddl_cur)
                Common.SelDdl(objread.ship_mode, ddl_sm)
                Common.SelDdl(objread.ship_term, ddl_st)
                Common.SelDdl(objread.pay_term, ddl_pt)
                Common.SelDdl(objread.pay_type, ddl_pm)
                Common.SelDdl(objread.del_code, ddl_dt)
            ...
            Else
                ...
                objread.RFQ_Name = ""
                objrfq_Ext.read_user(objread, Session("RFQ_Name"))
                'Else
                'Common.SelDdl(objread.ship_mode, ddl_sm)
                'Common.SelDdl(objread.ship_term, ddl_st)
            End If
    ...
    Private Sub dgviewitem_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.DataGridItemEventArgs) Handles dgviewitem.ItemDataBound
        If (e.Item.ItemType = ListItemType.Item Or e.Item.ItemType = ListItemType.AlternatingItem) Then
            ...
            Dim ddl_stk As New DropDownList
            ...
            ddl_stk = e.Item.FindControl("ddl_stock")
            'lbl_stk = e.Item.FindControl("lbl_stock")
            ...
            LoadStkDdl(ddl_stk)
            Common.SelDdl(Common.parseNull(dv("RD_ITEM_TYPE")), ddl_stk)

            If dv("PM_PRODUCT_FOR") = "B" Then
                ddl_stk.Enabled = False
            Else
                ddl_stk.Enabled = True
            End If

            'If dv("PM_PRODUCT_FOR") = "B" Then
            '    ddl_stk.Visible = False
            '    lbl_stk.Visible = True

            '    If Common.parseNull(dv("RD_ITEM_TYPE")) = "SP" Then
            '        lbl_stk.Text = "Spot"
            '    ElseIf Common.parseNull(dv("RD_ITEM_TYPE")) = "ST" Then
            '        lbl_stk.Text = "Stock"
            '    Else
            '        lbl_stk.Text = "MRO"
            '    End If
            'Else
            '    ddl_stk.Visible = True
            '    lbl_stk.Visible = False
            ...
            If Session("strItem") IsNot Nothing Then
                ...
                If (CInt(intRow)) <= strItem.Count - 1 Then
                    ...
                    ddl_stock.SelectedIndex = strItem(CInt(intRow))(3)
                End If
            End If
            ...
            If ViewState("modePR") = "pr" Then
                ...
                e.Item.Cells(ItemEnum.stk_type).Enabled = False
            End If
        End If
    End Sub
    ...
    Private Function validateInputs(ByRef strMsg As String) As Boolean
        ...
        Dim dgItem As DataGridItem
        Dim b, i As Integer
        Dim strOversea, strOversea2, strDelOversea, strItemType, strItemType2 As String
        Dim ddl_stk As DropDownList
        'Dim lbl_stock As Label
        Dim objDb As New EAD.DBCom
        ...
        For b = 0 To dgviewitem.Items.Count() - 1
            ddl_stk = dgviewitem.Items(b).Cells(11).FindControl("ddl_stock")

            If dgviewitem.Items(b).Cells(12).Text <> "B" And ddl_stk.SelectedValue = "" Then
                strMsg = "<ul type='disc'>"
                strMsg &= "<li>Item Type " & a.GetErrorMessage("00001") & "<ul type='disc'></ul></li>"
                strMsg &= "</ul>"
                Return False
            End If
        Next

        If ddl_cur.SelectedIndex = 0 Then
            strMsg = "<ul type='disc'>"
            strMsg &= "<li>Currency " & a.GetErrorMessage("00001") & "<ul type='disc'></ul></li>"
            strMsg &= "</ul>"
            Return False
        End If

        If ddl_dt.SelectedIndex = 0 Then
            strMsg = "<ul type='disc'>"
            strMsg &= "<li>Delivery Term " & a.GetErrorMessage("00001") & "<ul type='disc'></ul></li>"
            strMsg &= "</ul>"
            Return False
        Else


            For i = 0 To dgviewitem.Items.Count() - 1
                If i = 0 Then
                    If dgviewitem.Items(i).Cells(12).Text = "B" Or dgviewitem.Items(i).Cells(12).Text = "V" Then
                        strOversea = objDb.GetVal("SELECT PM_OVERSEA FROM PRODUCT_MSTR WHERE PM_PRODUCT_CODE='" & dgviewitem.Items(i).Cells(6).Text & "' ")
                    Else
                        strOversea = objDb.GetVal("SELECT CDT_DEL_OVERSEA FROM COMPANY_DELIVERY_TERM WHERE CDT_DEL_CODE='" & Common.Parse(ddl_dt.SelectedValue) & "' AND CDT_COY_ID='" & Session("CompanyId") & "' ")
                    End If
                Else
                    If dgviewitem.Items(i).Cells(12).Text = "B" Or dgviewitem.Items(i).Cells(12).Text = "V" Then
                        strOversea2 = objDb.GetVal("SELECT PM_OVERSEA FROM PRODUCT_MSTR WHERE PM_PRODUCT_CODE='" & dgviewitem.Items(i).Cells(6).Text & "' ")
                    Else
                        strOversea2 = objDb.GetVal("SELECT CDT_DEL_OVERSEA FROM COMPANY_DELIVERY_TERM WHERE CDT_DEL_CODE='" & Common.Parse(ddl_dt.SelectedValue) & "' AND CDT_COY_ID='" & Session("CompanyId") & "' ")
                    End If

                    If strOversea <> strOversea2 Then
                        strMsg = "<ul type='disc'>"
                        strMsg &= "<li>" & a.GetErrorMessage("00365") & "<ul type='disc'></ul></li>"
                        strMsg &= "</ul>"
                        Return False
                    End If
                End If

                strDelOversea = objDb.GetVal("SELECT CDT_DEL_OVERSEA FROM COMPANY_DELIVERY_TERM WHERE CDT_DEL_CODE='" & Common.Parse(ddl_dt.SelectedValue) & "' AND CDT_COY_ID='" & Session("CompanyId") & "' ")

                If strOversea <> strDelOversea Then
                    strMsg = "<ul type='disc'>"
                    strMsg &= "<li>" & a.GetErrorMessage("00365") & "<ul type='disc'></ul></li>"
                    strMsg &= "</ul>"
                    Return False
        ...
        If ddl_cur.SelectedIndex > 0 And ddl_dt.SelectedIndex > 0 Then
            'If ddl_cur.SelectedValue = "MYR" And ddl_dt.SelectedValue <> "3233" Then
            '    strMsg = "<ul type='disc'>"
            '    strMsg &= "<li>Only delivery term '3233 (DELIVERED SEH WAREHOUSE)' is allowed to use for MYR Currency.<ul type='disc'></ul></li>"
            '    strMsg &= "</ul>"
            '    Return False

            'ElseIf ddl_cur.SelectedValue <> "MYR" And ddl_dt.SelectedValue = "3233" Then
            '    strMsg = "<ul type='disc'>"
            '    strMsg &= "<li>Only MYR Currency with delivery term '3233 (DELIVERED SEH WAREHOUSE)' is allowed.<ul type='disc'></ul></li>"
            '    strMsg &= "</ul>"
            '    Return False
            ...
            If ddl_cur.SelectedValue <> "MYR" And ddl_dt.SelectedValue = "3233" Then
                strMsg = "<ul type='disc'>"
                strMsg &= "<li>" & a.GetErrorMessage("00364") & "<ul type='disc'></ul></li>"
                strMsg &= "</ul>"
                Return False
            End If
        End If
        ...
        For i = 0 To dgviewitem.Items.Count() - 1
            ddl_stk = dgviewitem.Items(i).Cells(11).FindControl("ddl_stock")
            'lbl_stock = dgviewitem.Items(i).Cells(11).FindControl("lbl_stock")

            If i = 0 Then
                strItemType = ddl_stk.SelectedItem.Text
                'If dgviewitem.Items(i).Cells(12).Text <> "B" Then
                '    strItemType = ddl_stk.SelectedItem.Text
                'Else
                '    strItemType = lbl_stock.Text
                'End If
            Else
                strItemType2 = ddl_stk.SelectedItem.Text
                'If dgviewitem.Items(i).Cells(12).Text <> "B" Then
                '    strItemType2 = ddl_stk.SelectedItem.Text
                'Else
                '    strItemType2 = lbl_stock.Text
                'End If

                If strItemType <> strItemType2 Then
                    strMsg = "<ul type='disc'>"
                    strMsg &= "<li>" & a.GetErrorMessage("00366") & "<ul type='disc'></ul></li>"
                    strMsg &= "</ul>"
                    Return False
                End If
            End If
    ...
    Sub save_data(ByVal strFrm As String)
        ...
        value(15) = 3
        value(16) = 0
        value(17) = Me.ddl_dt.SelectedItem.Value
        ...
        If strFrm <> "Submit" Then
            comfirmstatus = objRFQ_Ext.save_RFQ(strVendorList, value, rfq_num, rfq_id, dt, dtList, dtDetails, False)
        Else
            comfirmstatus = objRFQ_Ext.save_RFQ(strVendorList, value, rfq_num, rfq_id, dt, dtList, dtDetails, True)
        End If
...
checkMsg:
    ...
    Protected Sub cmd_Submit_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmd_Submit.Click
        ...
        If Page.IsValid And validateInputs(strMsg) Then
            ...
            Else
                ...
                '        If dgviewitem.Items(i).Cells(12).Text <> "B" Then
                '            If ddl_stk.SelectedValue <> "ST" Then
                '                blnContinue = True
                '                Exit For
                '            End If
                '        Else
                '            If lbl_stock.Text <> "ST" Then
                '                blnContinue = True
                '                Exit For
                '            End If
                '        End If
                ...
                '    strscript.Append("<script language=""javascript"">")
                '    strscript.Append("ans=confirm('Delivery Term is only applicable for stock item. Do you want to proceed?');")
                '    strscript.Append("if (ans){document.getElementById('hidBtnContinue').click();")
                '    strscript.Append("document.getElementById('cmd_Submit').style.display='none';")
                '    strscript.Append("document.getElementById('cmd_save').style.display='none';")
                '    strscript.Append("if (document.getElementById('cmd_addcath')){ document.getElementById('cmd_addcath').style.display='none';}")
                '    strscript.Append("if (document.getElementById('cmd_delete')){ document.getElementById('cmd_delete').style.display='none';}")
                '    strscript.Append("}")
    ...
    Private Sub cmd_delete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmd_delete.Click
        ...
        For Each dgItem In dgviewitem.Items
            ...
            If chkItem.Checked Then
            Else
                strItem.Add(New String() {txt_qty.Text, txt_delivery.Text, txt_warranty.Text, ddl_stock.SelectedIndex})
            End If
        Next
    ...
    Private Function bindRFQDetail() As DataTable
        ...
        Dim ddlItemType As DropDownList
        Dim lblUOM As Label
        ...
        For Each dgItem In dgviewitem.Items
            ...
            ddlItemType = dgItem.FindControl("ddl_stock")
            'lblItemType = dgItem.FindControl("lbl_stock")
            ...
            dr("ItemType") = ddlItemType.SelectedItem.Value
            'If dgItem.Cells(ItemEnum.Prod_for).Text <> "B" Then
            '    dr("ItemType") = ddlItemType.SelectedItem.Value
            'Else
            '    If lblItemType.Text = "Spot" Then
            '        dr("ItemType") = "SP"
            '    ElseIf lblItemType.Text = "Stock" Then
            '        dr("ItemType") = "ST"
            '    Else
            '        dr("ItemType") = "MI"
            '    End If
            '    'dr("ItemType") = lblItemType.Text
            'End If
    ...
    'Private Sub hidBtnContinue_ServerClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles hidBtnContinue.ServerClick
