...
#End Region
    ...
    Private Function Bindgrid(ByVal dgid As DataGrid, ByVal turn As String, Optional ByVal pSorted As Boolean = False) As String
        ...
        If turn = "1" Then
            ds = objPO.getlineitem(Me.lblPoNo.Text, Session("side"), False, Request(Trim("BCoyID")), False)

        ElseIf turn = "2" Then
            ds = objPO.get_docitem(Me.lblPoNo.Text, Session("side"), Request(Trim("BCoyID")))
        ElseIf turn = "3" Then
            ds = objPO.get_CRView(Me.lblPoNo.Text, "", Session("side"), Request(Trim("BCoyID")), "")
        ElseIf turn = "4" Then
        ...
        If (e.Item.ItemType = ListItemType.Item Or e.Item.ItemType = ListItemType.AlternatingItem) Then
            ...
            If Common.parseNull(dv("DOM_DO_NO")) <> "" Then
                ...
                If Session("side") = "b" Or Session("side") = "others" Or Session("side") = "u" Or (Session("side") = "other" And Request.QueryString("caller") = "buyer") Or (Session("side") = "other" And Request.QueryString("caller") = "") Then
                    ...
                    e.Item.Cells(EnumDoc.icDONo).Text = "<A href=""#"" onclick=""PopWindow('" & dDispatcher.direct("Report", "PreviewDO.aspx", "pageid=" & strPageId & "&DONo=" & dv("DOM_DO_NO") & "&SCoyID=" & dv("DOM_S_COY_ID")) & "')"" ><font color=#0000ff>" & Common.parseNull(dv("DOM_DO_NO")) & "</font></A>"
