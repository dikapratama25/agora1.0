...
#End Region
        ...
        If ValidateFields(strErrMsg) Then
            If Page.IsValid Then
                If ViewState("Mode") = "add" Then
                    ...
                    'txtUser.Enabled = False
                ElseIf ViewState("Mode") = "modify" Then
                    If objUser.UpdateUser(objUserDetail, False, EnumAppPackage.eProcure.ToString, False) Then
    ...
    Private Function GetInput() As User
        ...
        If Session("Env") <> "FTN" Then
            'Dim spUsrGrp As Array
            'Dim i As Integer
            'Dim blnMassAppr As Boolean
            'Dim blnInvMassAppr As Boolean
            ...
            'For i = 0 To spUsrGrp.Length - 1
            '    strTmpGrp = objUser.GetFixedRole(spUsrGrp(i))
            '    If strTmpGrp = Replace(FixedRole.Approving_Officer.ToString, "_", " ") Then
            '        blnMassAppr = True

            '    ElseIf strTmpGrp = Replace(FixedRole.Finance_Officer.ToString, "_", " ") Then
            '        blnInvMassAppr = True
            '    End If
            'Next
            If ViewState("side") = "B" Then
                ...
                lbl1.Visible = False
                chkMassAppr.Visible = False
                lbl2.Visible = False
                chkInvMassAppr.Visible = False
                'If Not objUser.IsRoleSelected(objUserDetail.UserGroup, FixedRole.Approving_Officer) And Not objUser.IsRoleSelected(objUserDetail.UserGroup, FixedRole.Finance_Officer) Then
                ...
                Else
                    chkInvMassAppr.Checked = False
                End If
                'If blnMassAppr <> True And blnInvMassAppr <> True Then
                '    trMassAppr.Visible = False
                '    chkMassAppr.Checked = False
                '    chkInvMassAppr.Checked = False
                'End If
                'If blnMassAppr = True Then  'AO
                '    trMassAppr.Visible = True
                '    lbl1.Text = "Mass Approval authorise approving officer to approve more than one PR/PO at a time"
                '    lbl1.Visible = True
                '    chkMassAppr.Visible = True
                ...
                'End If
                'If blnInvMassAppr = True Then   'FO
                '    trMassAppr.Visible = True
                '    lbl2.Text = "Mass Approval authorise approving officer to approve more than one Invoice at a time"
                '    lbl2.Visible = True
                '    chkInvMassAppr.Visible = True
